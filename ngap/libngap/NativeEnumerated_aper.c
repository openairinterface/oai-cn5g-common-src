...
asn_dec_rval_t NativeEnumerated_decode_aper(
  ...
  if (ct && ct->upper_bound >= 255) {
    ...
    ASN_DEBUG(
        "For NativeEnumerated %s,offset = %zu Padding bits = %d", td->name,
        pd->moved, padding);
    ASN_DEBUG(
        "For NativeEnumerated %s, upper bound = %llu", td->name,
        (unsigned long long) ct->upper_bound);
    if (padding > 0) per_get_few_bits(pd, padding);
  }

  if (ct && ct->range_bits >= 0) {
    value = per_get_few_bits(pd, ct->range_bits);
    if (value < 0) ASN__DECODE_STARVED;
    if (value >= (specs->extension ? specs->extension - 1 : specs->map_count))
      ASN__DECODE_FAILED;
  } else {
    if (!specs->extension) ASN__DECODE_FAILED;
    /*
...
asn_enc_rval_t NativeEnumerated_encode_aper(
  ...
  kf            = bsearch(
      &key, specs->value2enum, specs->map_count, sizeof(key),
  ...
  if (ct->range_bits >= 0) {
    int cmpWith = specs->extension ? specs->extension - 1 : specs->map_count;
  ...
  if (range_bits && ct && ct->range_bits >= 0) {
    if (per_put_few_bits(po, value, ct->range_bits)) ASN__ENCODE_FAILED;
    ASN__ENCODED_OK(er);
  }

  if (!specs->extension) ASN__ENCODE_FAILED;
