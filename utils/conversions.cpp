...
struct in_addr conv::fromString(const std::string &addr4) {
  ...
  if (ret != 1) {
    logger_common::common().error(__PRETTY_FUNCTION__ +
                                  std::string{" Failed to convert "} + addr4);
  }
...
//------------------------------------------------------------------------------
bool conv::string_to_int(const std::string &str, uint32_t &value,
                         uint8_t base) {
  if (str.empty())
    return false;
  ...
  if (base == 16) {
    if (str.size() <= 2)
      return false;
    if (!boost::iequals(str.substr(0, 2), "0x"))
      return false;
  }
  try {
...
//------------------------------------------------------------------------------
void conv::int_to_string_hex(uint64_t value, std::string &value_str,
                             uint8_t length) {
...
//------------------------------------------------------------------------------
void conv::get_amf_id(uint8_t amf_region_id, uint16_t amf_set_id,
                      uint8_t amf_pointer, uint32_t &amf_id) {
  // AMF Region ID: 8bits
  // AMF Set ID: 10 bits
  // AMF Pointer: 6 bits
...
//------------------------------------------------------------------------------
void conv::get_amf_id(uint8_t amf_region_id, uint16_t amf_set_id,
                      uint8_t amf_pointer, std::string &amf_id) {
  // AMF Region ID: 8bits
  // AMF Set ID: 10 bits
  // AMF Pointer: 6 bits
...
//------------------------------------------------------------------------------
void conv::get_amf_id(const std::string &amf_region_id,
                      const std::string &amf_set_id,
                      const std::string &amf_pointer, uint32_t &amf_id) {
  uint8_t amf_region_id_int = {};
  uint16_t amf_set_id_int = {};
  uint8_t amf_pointer_int = {};

  get_amf_id(string_hex_to_int(amf_region_id), string_hex_to_int(amf_set_id),
...
//------------------------------------------------------------------------------
void conv::convert_string_2_hex(const std::string &input_str,
  ...
  oai::utils::output_wrapper::print_buffer({}, "Data input", data,
                                           input_str.length());
...
//------------------------------------------------------------------------------
nlohmann::json conv::yaml_to_json(const YAML::Node &node,
                                  bool parse_hex_values) {
...
//------------------------------------------------------------------------------
void conv::string_to_uint_mac_address(const std::string &mac_address_str,
                                      uint8_t mac_address[6], char delimiter) {
