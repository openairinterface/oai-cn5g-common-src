...
NgEnbId::NgEnbId() { m_Present = Ngap_NgENB_ID_PR_NOTHING; }
...
bool NgEnbId::encode(Ngap_NgENB_ID_t &ng_enb_id) const {
  ...
  switch (m_Present) {
  case Ngap_NgENB_ID_PR_macroNgENB_ID: {
    INT24_TO_BIT_STRING(m_Id.macro_ngenb_id && 0x00ffffff,
                        &ng_enb_id.choice.macroNgENB_ID, 20);
  } break;
  case Ngap_NgENB_ID_PR_shortMacroNgENB_ID: {
    INT24_TO_BIT_STRING(m_Id.short_macro_ngENB_id && 0x00ffffff,
                        &ng_enb_id.choice.shortMacroNgENB_ID, 18);
  } break;
  case Ngap_NgENB_ID_PR_longMacroNgENB_ID: {
    INT24_TO_BIT_STRING(m_Id.long_macro_ngENB_id && 0x00ffffff,
...
bool NgEnbId::decode(const Ngap_NgENB_ID_t &ng_enb_id) {
  ...
  case Ngap_NgENB_ID_PR_shortMacroNgENB_ID: {
    BIT_STRING_TO_INT24(&ng_enb_id.choice.shortMacroNgENB_ID,
                        m_Id.short_macro_ngENB_id);
  } break;
  case Ngap_NgENB_ID_PR_longMacroNgENB_ID: {
    BIT_STRING_TO_INT24(&ng_enb_id.choice.shortMacroNgENB_ID,
                        m_Id.long_macro_ngENB_id);
  } break;
  default: {
    m_Present = Ngap_NgENB_ID_PR_NOTHING;
  }
