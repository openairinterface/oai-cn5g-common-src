...
asn_TYPE_operation_t asn_OP_OCTET_STRING = {
    ...
    OCTET_STRING_decode_xer_auto,   /* Auto-detect hex or Base64 format for
                                       decoding */
    OCTET_STRING_encode_xer_base64, /* Per X.693, Base64 is the default encoding
                                       for OCTET STRING */
#else
    0,
    0,
...
OCTET_STRING_t* OCTET_STRING_new_fromBuf(
    const asn_TYPE_descriptor_t* td, const char* str, int len) {
  const asn_OCTET_STRING_specifics_t* specs =
...
int OCTET_STRING_per_get_characters(
  ...
  ASN_DEBUG(
      "Expanding %d characters into (%ld..%ld):%d", (int) units, lb, ub,
      unit_bits);
...
int OCTET_STRING_per_put_characters(
  ...
  ASN_DEBUG(
      "Squeezing %d characters into (%ld..%ld):%d (%d bpc)", (int) units, lb,
      ub, unit_bits, bpc);
  ...
  for (ub -= lb; buf < end; buf += bpc) {
    ...
    if (ch < 0 || ch > ub) {
      ASN_DEBUG(
          "Character %d (0x%02x) is out of range (%ld..%ld)", *buf, value, lb,
          ub + lb);
      return -1;
    }
