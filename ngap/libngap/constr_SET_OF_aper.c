...
asn_enc_rval_t SET_OF_encode_aper(const asn_TYPE_descriptor_t *td,
  ...
  if (ct) {
    ...
    ASN_DEBUG("lb %lld ub %lld %s", (long long int)ct->lower_bound,
...
asn_dec_rval_t SET_OF_decode_aper(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  if (ct && ct->effective_bits >= 0) {
    ...
    ASN_DEBUG("Preparing to fetch %ld+%lld elements from %s", (long)nelems,
