...
bool UEContextRelease::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "UEContextRelease" : pathPrefix;
    ...
    if (!m_NgapCause.validate(msg, _pathPrefix + ".ngapCause")) {
        msg << _pathPrefix << ": NgapCause is invalid;";
        success = false;
    }
    return success;
}
...
oai::model::NgApCause UEContextRelease::getNgapCause() const
{
    return m_NgapCause;
}
void UEContextRelease::setNgapCause(oai::model::NgApCause const& value)
{
    m_NgapCause = value;
}
