...
void ExtendedProtocolConfigurationOptions::SetLength() {
  ...
  if (protocol_or_container_ids.size() > 0) {
    for (auto p : protocol_or_container_ids) {
      length += (p.length_of_protocol_id_contents +
...
void ExtendedProtocolConfigurationOptions::GetProtocolOrContainerIds(
  ...
  ids.assign(protocol_or_container_ids.begin(),
             protocol_or_container_ids.end());
}
...
void ExtendedProtocolConfigurationOptions::Set(
  ...
  protocol_or_container_ids.assign(conf_opt.protocol_or_container_ids.begin(),
...
void ExtendedProtocolConfigurationOptions::Get(
  ...
  conf_opt.protocol_or_container_ids.assign(protocol_or_container_ids.begin(),
                                            protocol_or_container_ids.end());
...
ExtendedProtocolConfigurationOptions::Get() const {
...
int ExtendedProtocolConfigurationOptions::Encode(uint8_t *buf, int len) const {
  ...
  for (int i = 0; i < protocol_or_container_ids.size(); i++) {
    ENCODE_U16(buf + encoded_size, protocol_or_container_ids[i].protocol_id,
               encoded_size);
...
//------------------------------------------------------------------------------
int ExtendedProtocolConfigurationOptions::Decode(const uint8_t *const buf,
                                                 int len, bool is_iei) {
  ...
  while (decoded_size < GetIeLength()) {
    ...
    DECODE_U8(buf + decoded_size, pco_id.length_of_protocol_id_contents,
              decoded_size);
    if (pco_id.length_of_protocol_id_contents > 0) {
      bstring b_str = nullptr;

      int decoded_bstring_size =
          decode_bstring(&b_str, pco_id.length_of_protocol_id_contents,
    ...
    oai::logger::logger_common::nas().debug("Decoded PCO ID 0x%x",
                                            pco_id.protocol_id);
