        ...
        if(ct->range_bits >= 0) {
            if (ct->range_bits > 16) {
                ...
                if(ct->upper_bound < ct->lower_bound) {
                    ...
                    uintmax_t range;
                    if(ct->lower_bound >= 0) {
                        range = (uintmax_t)ct->upper_bound
                                - (uintmax_t)ct->lower_bound;
                    } else {
                        uintmax_t neg_lb =
                            (uintmax_t)(-(ct->lower_bound + 1)) + 1;
                        if((uintmax_t)ct->upper_bound
                           > UINTMAX_MAX - neg_lb)
                            ASN__DECODE_FAILED;
                ...
                if(ct->lower_bound >= 0) {
                    if(offset > (uintmax_t)INTMAX_MAX
                                - (uintmax_t)ct->lower_bound)
                        ASN__DECODE_FAILED;
                    ...
                    if(offset >= neg_lb) {
                        ...
                        if(positive > (uintmax_t)INTMAX_MAX)
                            ASN__DECODE_FAILED;
...
INTEGER_encode_aper(const asn_TYPE_descriptor_t *td,
    ...
    if(ct && ct->range_bits >= 0) {
        ...
        if (ct->range_bits < 8 || (ct->range_bits == 8 && range < 256)) {
            ...
            if(num_bytes > max_range_bytes)
                ASN__ENCODE_FAILED;

            while(((size_t)1 << length_bits) < max_range_bytes)
                length_bits++;
