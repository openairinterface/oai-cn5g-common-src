...
asn_dec_rval_t SEQUENCE_decode_uper(const asn_codec_ctx_t *opt_codec_ctx,
...
static int SEQUENCE__handle_extensions(const asn_TYPE_descriptor_t *td,
                                       const void *sptr, asn_per_outp_t *po1,
                                       asn_per_outp_t *po2) {
  const asn_SEQUENCE_specifics_t *specs =
      (const asn_SEQUENCE_specifics_t *)td->specifics;
  int exts_present = 0;
  ...
  for (edx = specs->first_extension; edx < td->elements_count; edx++) {
    ...
    if (po2 && present &&
        uper_open_type_put(elm->type, elm->encoding_constraints.per_constraints,
                           *memb_ptr2, po2))
      return -1;
  }
...
asn_enc_rval_t SEQUENCE_encode_uper(const asn_TYPE_descriptor_t *td,
                                    const asn_per_constraints_t *constraints,
