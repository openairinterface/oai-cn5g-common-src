...
bool HttpMethod_anyOf::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "HttpMethod_anyOf" : pathPrefix;

    
    if (m_value == HttpMethod_anyOf::eHttpMethod_anyOf::INVALID_VALUE_OPENAPI_GENERATED)
    {
        success = false;
