...
int CHOICE_constraint(const asn_TYPE_descriptor_t *td, const void *sptr,
  ...
  if (present > 0 && present <= td->elements_count) {
    ...
    if (elm->flags & ATF_POINTER) {
      ...
      if (!memb_ptr) {
        ...
        ASN__CTFAIL(app_key, td, sptr,
                    "%s: mandatory CHOICE element %s absent (%s:%d)", td->name,
    ...
    ASN__CTFAIL(app_key, td, sptr, "%s: no CHOICE element given (%s:%d)",
                td->name, __FILE__, __LINE__);
    return -1;
  }
}

void CHOICE_free(const asn_TYPE_descriptor_t *td, void *ptr,
 ...
 * produce packed code.
 */
unsigned _fetch_present_idx(const void *struct_ptr, unsigned pres_offset,
                            unsigned pres_size) {
...
void _set_present_idx(void *struct_ptr, unsigned pres_offset,
                      unsigned pres_size, unsigned present) {
  void *present_ptr;
...
static const void *_get_member_ptr(const asn_TYPE_descriptor_t *td,
                                   const void *sptr,
                                   asn_TYPE_member_t **elm_ptr,
                                   unsigned *present_out) {
  const asn_CHOICE_specifics_t *specs =
      (const asn_CHOICE_specifics_t *)td->specifics;
  unsigned present;
...
int CHOICE_compare(const asn_TYPE_descriptor_t *td, const void *aptr,
                   const void *bptr) {
 ...
 */
int CHOICE_variant_set_presence(const asn_TYPE_descriptor_t *td, void *sptr,
                                unsigned present) {
  const asn_CHOICE_specifics_t *specs =
      (const asn_CHOICE_specifics_t *)td->specifics;
