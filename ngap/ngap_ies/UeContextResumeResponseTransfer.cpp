...
//------------------------------------------------------------------------------
int UeContextResumeResponseTransfer::encode(uint8_t* buf, int buf_size) {
  ngap_utils::print_asn_msg(
      &asn_DEF_Ngap_UEContextResumeResponseTransfer,
      m_UeContextResumeResponseTransferIe);
  asn_enc_rval_t er = aper_encode_to_buffer(
      &asn_DEF_Ngap_UEContextResumeResponseTransfer, NULL,
      m_UeContextResumeResponseTransferIe, buf, buf_size);
  oai::logger::logger_common::ngap().debug("er.encoded %d", er.encoded);
  return er.encoded;
}

//------------------------------------------------------------------------------
bool UeContextResumeResponseTransfer::decode(uint8_t* buf, int buf_size) {
  asn_dec_rval_t rc = asn_decode(
      NULL, ATS_ALIGNED_CANONICAL_PER,
      &asn_DEF_Ngap_UEContextResumeResponseTransfer,
      (void**) &m_UeContextResumeResponseTransferIe, buf, buf_size);
  if (rc.code == RC_OK) {
