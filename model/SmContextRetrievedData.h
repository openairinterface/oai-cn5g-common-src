/**
* Nsmf_PDUSession
* SMF PDU Session Service.   © 2025, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.7
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * SmContextRetrievedData.h
 *
 * Data within Retrieve SM Context Response
 */

#ifndef SmContextRetrievedData_H_
#define SmContextRetrievedData_H_


#include "AfCoordinationInfo.h"
#include "SmContext.h"
#include <string>
#include "SmallDataRateStatus.h"
#include "ApnRateStatus.h"
#include <nlohmann/json.hpp>

namespace oai::model
{

/// <summary>
/// Data within Retrieve SM Context Response
/// </summary>
class  SmContextRetrievedData
{
public:
    SmContextRetrievedData();
    virtual ~SmContextRetrievedData() = default;


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const SmContextRetrievedData& rhs) const;
    bool operator!=(const SmContextRetrievedData& rhs) const;

    /////////////////////////////////////////////
    /// SmContextRetrievedData members

    /// <summary>
    /// UE EPS PDN Connection container from SMF to AMF
    /// </summary>
    std::string getUeEpsPdnConnection() const;
    void setUeEpsPdnConnection(std::string const& value);
    /// <summary>
    /// 
    /// </summary>
    oai::model::SmContext getSmContext() const;
    void setSmContext(oai::model::SmContext const& value);
    bool smContextIsSet() const;
    void unsetSmContext();
    /// <summary>
    /// 
    /// </summary>
    oai::model::SmallDataRateStatus getSmallDataRateStatus() const;
    void setSmallDataRateStatus(oai::model::SmallDataRateStatus const& value);
    bool smallDataRateStatusIsSet() const;
    void unsetSmallDataRateStatus();
    /// <summary>
    /// 
    /// </summary>
    oai::model::ApnRateStatus getApnRateStatus() const;
    void setApnRateStatus(oai::model::ApnRateStatus const& value);
    bool apnRateStatusIsSet() const;
    void unsetApnRateStatus();
    /// <summary>
    /// 
    /// </summary>
    bool isDlDataWaitingInd() const;
    void setDlDataWaitingInd(bool const value);
    bool dlDataWaitingIndIsSet() const;
    void unsetDlDataWaitingInd();
    /// <summary>
    /// 
    /// </summary>
    oai::model::AfCoordinationInfo getAfCoordinationInfo() const;
    void setAfCoordinationInfo(oai::model::AfCoordinationInfo const& value);
    bool afCoordinationInfoIsSet() const;
    void unsetAfCoordinationInfo();

    friend  void to_json(nlohmann::json& j, const SmContextRetrievedData& o);
    friend  void from_json(const nlohmann::json& j, SmContextRetrievedData& o);
protected:
    std::string m_UeEpsPdnConnection;

    oai::model::SmContext m_SmContext;
    bool m_SmContextIsSet;
    oai::model::SmallDataRateStatus m_SmallDataRateStatus;
    bool m_SmallDataRateStatusIsSet;
    oai::model::ApnRateStatus m_ApnRateStatus;
    bool m_ApnRateStatusIsSet;
    bool m_DlDataWaitingInd;
    bool m_DlDataWaitingIndIsSet;
    oai::model::AfCoordinationInfo m_AfCoordinationInfo;
    bool m_AfCoordinationInfoIsSet;
    
};

} // namespace oai::model

#endif /* SmContextRetrievedData_H_ */
