...
int uper_open_type_put(const asn_TYPE_descriptor_t *td,
  ...
  ASN_DEBUG("Open type put %s of length %" ASN_PRI_SSIZE " + overhead (1byte?)",
            td->name, size);
  ...
  do {
    ...
    if (may_save < 0)
      break;
    if (per_put_many_bits(po, bptr, may_save * 8))
      break;
...
static asn_dec_rval_t uper_open_type_get_simple(
  ...
  do {
    ...
    if (per_get_many_bits(pd, (buf == NULL) ? NULL : buf + bufLen, 0,
                          chunk_bytes << 3)) {
      FREEMEM(buf);
      ASN__DECODE_STARVED;
    }
...
static asn_dec_rval_t CC_NOTUSED uper_open_type_get_complex(
    const asn_codec_ctx_t *ctx, const asn_TYPE_descriptor_t *td,
  ...
  ASN_DEBUG("Getting open type %s from %s", td->name, asn_bit_data_string(pd));
  arg.oldpd = *pd;
  arg.unclaimed = 0;
  arg.ot_moved = 0;
  arg.repeat = 1;
...
#define UPDRESTOREPD                                                           \
  ...
  if (padding) {
    ...
    case 0:
      break;
    default:
      ASN_DEBUG("Non-blank padding (%d bits 0x%02x)", (int)padding,
                (int)pvalue);
      UPDRESTOREPD;
      ASN__DECODE_FAILED;
    }
...
asn_dec_rval_t uper_open_type_get(const asn_codec_ctx_t *ctx,
...
int uper_open_type_skip(const asn_codec_ctx_t *ctx, asn_per_data_t *pd) {
...
static int uper_ugot_refill(asn_per_data_t *pd) {
  ...
  ASN_DEBUG("REFILLING pd->moved=%ld, oldpd->moved=%ld", (long)pd->moved,
            (long)oldpd->moved);
  ...
  if (arg->unclaimed) {
    ...
    ASN_DEBUG("UNCLAIMED <- return from (pd->moved=%ld)", (long)pd->moved);
    return 0;
  }
  ...
  if (avail >= next_chunk_bits) {
    ...
    ASN_DEBUG("!+Parent frame %ld bits, alloting %ld [%ld..%ld] (%ld)",
              (long)next_chunk_bits, (long)oldpd->moved, (long)oldpd->nboff,
              (long)oldpd->nbits, (long)(oldpd->nbits - oldpd->nboff));
  } else {
    pd->nbits = oldpd->nbits;
    arg->unclaimed = next_chunk_bits - avail;
    ASN_DEBUG("!-Parent frame %ld, require %ld, will claim %ld", (long)avail,
              (long)next_chunk_bits, (long)arg->unclaimed);
  }
  pd->buffer = oldpd->buffer;
  pd->nboff = oldpd->nboff;
  ASN_DEBUG("Refilled pd%s old%s", asn_bit_data_string(pd),
            asn_bit_data_string(oldpd));
  return 0;
}
