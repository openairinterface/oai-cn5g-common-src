...
asn_enc_rval_t SEQUENCE_encode_cbor(
...
asn_dec_rval_t SEQUENCE_decode_cbor(
  ...
  for (i = 0; i < map_count; i++) {
    ...
    key_hlen =
        cbor_decode_head(buf + consumed, size - consumed, &key_major, &key_len);
    ...
    for (edx = 0; edx < td->elements_count; edx++) {
      ...
      if (strlen(elm->name) == key_len &&
        ...
        if (elm->flags & ATF_OPEN_TYPE) {
          tmprval = OPEN_TYPE_cbor_get(
              opt_codec_ctx, td, st, elm, buf + consumed, size - consumed);
        } else {
          if (!elm->type->op->cbor_decoder) ASN__DECODE_FAILED;
          tmprval = elm->type->op->cbor_decoder(
              opt_codec_ctx, elm->type, memb_ptr2, buf + consumed,
              size - consumed);
        }
