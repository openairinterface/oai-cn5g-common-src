/**
* Npcf_BDTPolicyControl Service API
* PCF BDT Policy Control Service.   © 2022, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.0
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * Notification.h
 *
 * Describes a BDT notification.
 */

#ifndef Notification_H_
#define Notification_H_


#include "TransferPolicy.h"
#include <string>
#include "NetworkAreaInfo.h"
#include "TimeWindow.h"
#include <vector>
#include <nlohmann/json.hpp>

namespace oai::model
{

/// <summary>
/// Describes a BDT notification.
/// </summary>
class  Notification
{
public:
    Notification();
    virtual ~Notification() = default;


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const Notification& rhs) const;
    bool operator!=(const Notification& rhs) const;

    /////////////////////////////////////////////
    /// Notification members

    /// <summary>
    /// string identifying a BDT Reference ID as defined in clause 5.3.3 of 3GPP TS 29.154.
    /// </summary>
    std::string getBdtRefId() const;
    void setBdtRefId(std::string const& value);
    /// <summary>
    /// Contains a list of the candidate transfer policies from which the AF may select a new transfer policy due to a network performance is below the criteria set by the operator. 
    /// </summary>
    std::vector<oai::model::TransferPolicy> getCandPolicies() const;
    void setCandPolicies(std::vector<oai::model::TransferPolicy> const& value);
    bool candPoliciesIsSet() const;
    void unsetCandPolicies();
    /// <summary>
    /// 
    /// </summary>
    oai::model::NetworkAreaInfo getNwAreaInfo() const;
    void setNwAreaInfo(oai::model::NetworkAreaInfo const& value);
    bool nwAreaInfoIsSet() const;
    void unsetNwAreaInfo();
    /// <summary>
    /// 
    /// </summary>
    oai::model::TimeWindow getTimeWindow() const;
    void setTimeWindow(oai::model::TimeWindow const& value);
    bool timeWindowIsSet() const;
    void unsetTimeWindow();

    friend  void to_json(nlohmann::json& j, const Notification& o);
    friend  void from_json(const nlohmann::json& j, Notification& o);
protected:
    std::string m_BdtRefId;

    std::vector<oai::model::TransferPolicy> m_CandPolicies;
    bool m_CandPoliciesIsSet;
    oai::model::NetworkAreaInfo m_NwAreaInfo;
    bool m_NwAreaInfoIsSet;
    oai::model::TimeWindow m_TimeWindow;
    bool m_TimeWindowIsSet;
    
};

} // namespace oai::model

#endif /* Notification_H_ */
