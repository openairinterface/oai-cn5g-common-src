...
asn_dec_rval_t INTEGER_decode_uper(const asn_codec_ctx_t *opt_codec_ctx,
    ...
    if (ct->range_bits >= 0) {
      ...
      if (specs && specs->field_unsigned) {
        ...
        if (uper_get_constrained_whole_number(pd, &uvalue, ct->range_bits))
          ASN__DECODE_STARVED;
        ASN_DEBUG("Got value %lu + low %ld", uvalue, ct->lower_bound);
        ...
        if (uper_get_constrained_whole_number(pd, &uvalue, ct->range_bits))
          ASN__DECODE_STARVED;
        ASN_DEBUG("Got value %lu + low %ld", uvalue, ct->lower_bound);
        if (per_imax_range_unrebase(uvalue, ct->lower_bound, ct->upper_bound,
                                    &svalue) ||
...
asn_enc_rval_t INTEGER_encode_uper(const asn_TYPE_descriptor_t *td,
                                   const asn_per_constraints_t *constraints,
  ...
  if (ct) {
    ...
    if (specs && specs->field_unsigned) {
      ...
      ASN_DEBUG("Value %lu (%02x/%" ASN_PRI_SIZE ") lb %lu ub %lu %s", value.u,
                st->buf[0], st->size, ct->lower_bound, ct->upper_bound,
                inext ? "ext" : "fix");
    } else {
      ...
      if (ct->flags & APC_SEMI_CONSTRAINED) {
        ...
        if (value.s < ct->lower_bound || value.s > ct->upper_bound)
          inext = 1;
      }
      ASN_DEBUG("Value %ld (%02x/%" ASN_PRI_SIZE ") lb %ld ub %ld %s", value.s,
                st->buf[0], st->size, ct->lower_bound, ct->upper_bound,
                inext ? "ext" : "fix");
    }
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    if (specs && specs->field_unsigned) {
      ...
      if (per_imax_range_rebase(value.s, ct->lower_bound, ct->upper_bound,
                                &v)) {
        ASN__ENCODE_FAILED;
      }
    }
    ASN_DEBUG("Encoding integer %lu with range %d bits", v, ct->range_bits);
