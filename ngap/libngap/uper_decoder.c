...
asn_dec_rval_t uper_decode_canonical(
    const asn_codec_ctx_t* opt_codec_ctx, const asn_TYPE_descriptor_t* td,
    void** sptr, const void* buffer, size_t size, int skip_bits,
    int unused_bits) {
  asn_codec_ctx_t canonical_ctx;
  ...
  return uper_decode(
      &canonical_ctx, td, sptr, buffer, size, skip_bits, unused_bits);
}
...
asn_dec_rval_t uper_decode(
  ...
  if (rval.code == RC_OK) {
    ...
    ASN_DEBUG(
        "PER decoding consumed %ld, counted %ld", (long) rval.consumed,
        (long) pd.moved);
    ...
    rval.consumed =
        ((pd.buffer - (const uint8_t*) buffer) << 3) + pd.nboff - skip_bits;
    ASN_DEBUG(
        "PER decoding failed at bit %ld (byte %ld, bit %ld)",
