...
//------------------------------------------------------------------------------
void SNssai::setSst(const uint8_t &sst) { m_Sst = sst; }
//------------------------------------------------------------------------------
void SNssai::getSst(std::string &sst) const { sst = std::to_string(m_Sst); }

//------------------------------------------------------------------------------
std::string SNssai::getSstStr() const { return std::to_string(m_Sst); }

//------------------------------------------------------------------------------
void SNssai::getSst(uint8_t &sst) const { sst = m_Sst; }

//------------------------------------------------------------------------------
uint8_t SNssai::getSst() const { return m_Sst; }
...
//------------------------------------------------------------------------------
void SNssai::setSd(const uint32_t &sd) { m_Sd = std::optional<uint32_t>(sd); }
...
bool SNssai::decode(const Ngap_S_NSSAI_t &s_NSSAI) {
  if (!ngap_utils::octet_string_2_int8(s_NSSAI.sST, m_Sst))
    return false;
  if (s_NSSAI.sD) {
    if (!decodeSd(*s_NSSAI.sD))
      return false;
  }
  return true;
}
