 ...
 */
asn_enc_rval_t xer_encode(const asn_TYPE_descriptor_t *td, const void *sptr,
...
int xer_fprint(FILE *stream, const asn_TYPE_descriptor_t *td,
               const void *sptr) {
  asn_enc_rval_t er = {0, 0, 0};
...
static int xer__buffer_append(const void *buffer, size_t size, void *app_key) {
...
enum xer_equivalence_e xer_equivalent(const struct asn_TYPE_descriptor_s *td,
  ...
  if (!td || !struct1 || !struct2) {
    if (opt_debug_stream) {
      if (!td)
        fprintf(opt_debug_stream, "Type descriptor missing\n");
      if (!struct1)
        fprintf(opt_debug_stream, "Structure 1 missing\n");
      if (!struct2)
        fprintf(opt_debug_stream, "Structure 2 missing\n");
    }
    return XEQ_FAILURE;
  }
  ...
  rval = xer_decode(NULL, td, (void **)&sptr, xb1.buffer, xb1.buffer_size);
  switch (rval.code) {
  case RC_OK:
    break;
  case RC_WMORE:
  ...
  if (rval.consumed != xb1.buffer_size &&
    ...
    if (opt_debug_stream) {
      fprintf(opt_debug_stream,
              "Round-trip decode of %s required less bytes (%" ASN_PRI_SIZE
              ") than "
