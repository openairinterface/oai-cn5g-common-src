...
namespace oai::ngap {
//------------------------------------------------------------------------------
void ngap_utils::print_asn_msg(const asn_TYPE_descriptor_t *td,
...
//------------------------------------------------------------------------------
bool ngap_utils::octet_string_2_bstring(const OCTET_STRING_t &octet_str,
                                        bstring &b_str) {
  if (!octet_str.buf or (octet_str.size == 0))
    return false;
...
//------------------------------------------------------------------------------
bool ngap_utils::bstring_2_octet_string(const bstring &b_str,
...
//------------------------------------------------------------------------------
bool ngap_utils::bstring_2_bit_string(const bstring &b_str,
                                      BIT_STRING_t &bit_str) {
...
//------------------------------------------------------------------------------
void ngap_utils::octet_string_2_string(const OCTET_STRING_t &octet_str,
                                       std::string &str) {
  if (!octet_str.buf or (octet_str.size == 0))
    return;
...
//------------------------------------------------------------------------------
void ngap_utils::string_2_octet_string(const std::string &str,
                                       OCTET_STRING_t &o_str) {
...
//------------------------------------------------------------------------------
bool ngap_utils::octet_string_2_int8(const OCTET_STRING_t &o_str,
                                     uint8_t &value) {
  if (o_str.size != 1)
    return false;
  value = o_str.buf[0];
...
//------------------------------------------------------------------------------
bool ngap_utils::octet_string_copy(OCTET_STRING_t &destination,
                                   const OCTET_STRING_t &source) {
  if (!source.buf)
    return false;
...
//------------------------------------------------------------------------------
bool ngap_utils::string_2_masked_imeisv(const std::string &imeisv_str,
                                        BIT_STRING_t &imeisv) {
  int len = imeisv_str.length();
  if (len != 16)
    return false; // Must contain 16 digits
