...
public:
  ...
  [[nodiscard]] virtual std::shared_ptr<nf>
  get_nf(const std::string &nf_name) const = 0;
...
public:
  explicit config(const std::string &config_path, const std::string &nf_name,
  ...
  [[nodiscard]] std::shared_ptr<nf>
  get_nf(const std::string &nf_name) const override;
