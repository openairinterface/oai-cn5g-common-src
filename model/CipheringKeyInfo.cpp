...
bool CipheringKeyInfo::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "CipheringKeyInfo" : pathPrefix;

         
    
    /* CipheringData */ {
        const std::vector<oai::model::CipheringDataSet>& value = m_CipheringData;
        ...
        { // Recursive validation of array elements
            ...
            int i = 0;
            for (const oai::model::CipheringDataSet& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".cipheringData") && success;
...
std::vector<oai::model::CipheringDataSet> CipheringKeyInfo::getCipheringData() const
{
    return m_CipheringData;
}
void CipheringKeyInfo::setCipheringData(std::vector<oai::model::CipheringDataSet> const& value)
{
