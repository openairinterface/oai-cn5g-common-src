 ...
 */
ssize_t uper_get_length(asn_per_data_t *pd, int ebits, size_t lower_bound,
                        int *repeat) {
  ssize_t value;
...
int uper_get_constrained_whole_number(asn_per_data_t *pd, uintmax_t *out_value,
                                      int nbits) {
...
int uper_put_nslength(asn_per_outp_t *po, size_t length) {
  if (length <= 64) {
    ...
    int need_eom = 0;
    if (uper_put_length(po, length, &need_eom) != (ssize_t)length || need_eom) {
...
int per_long_range_rebase(long v, intmax_t lb, intmax_t ub,
