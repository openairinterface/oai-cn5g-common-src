 ...
 */
asn_enc_rval_t OCTET_STRING_encode_cbor_utf8(
 ...
 */
asn_dec_rval_t OCTET_STRING_decode_cbor_utf8(
  ...
  hlen = cbor_decode_head(
      buf + tag_skip, size - (size_t) tag_skip, &major, &str_len);
  ...
  if (str_len > 0) memcpy(p, buf + tag_skip + hlen, str_len);
  p[str_len] = '\0';
