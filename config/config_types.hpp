...
public:
  ...
  to_string(const std::string &indent) const = 0;
...
protected:
  ...
  static void get_ipv4_range(const in_addr &ipv4_subnet,
...
public:
  explicit string_config_value(const std::string &name,
...
protected:
  ...
  to_string_for_local(const std::string &indent) const;
...
public:
  explicit local_interface(const std::string &name, const std::string &host,
                           uint16_t port, const std::string &if_name);
...
public:
  explicit sbi_interface(const std::string &name, const std::string &host,
...
public:
  explicit nf(const std::string &name, const std::string &host,
...
public:
  explicit nf_features_config(const std::string &name,
...
protected:
  [[nodiscard]] int_config_value
  get_default_database_port(const string_config_value &database_type) const;
...
public:
  explicit ue_dns(const std::string &primary_dns_v4,
...
public:
  explicit dnn_config(const std::string &dnn, const std::string &pdu_type,
