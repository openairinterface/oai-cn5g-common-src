/**
* Nnef_EventExposure
* NEF Event Exposure Service.   © 2022 , 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.0
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * ExceptionId_anyOf.h
 *
 * 
 */

#ifndef ExceptionId_anyOf_H_
#define ExceptionId_anyOf_H_


#include <nlohmann/json.hpp>

namespace oai::_3gpp::model
{

/// <summary>
/// 
/// </summary>
class  ExceptionId_anyOf
{
public:
    ExceptionId_anyOf();
    virtual ~ExceptionId_anyOf() = default;

    enum class eExceptionId_anyOf {
    // To have a valid default value.
    // Avoiding name clashes with user defined
    // enum values
    INVALID_VALUE_OPENAPI_GENERATED = 0,
    UNEXPECTED_UE_LOCATION, 
    UNEXPECTED_LONG_LIVE_FLOW, 
    UNEXPECTED_LARGE_RATE_FLOW, 
    UNEXPECTED_WAKEUP, 
    SUSPICION_OF_DDOS_ATTACK, 
    WRONG_DESTINATION_ADDRESS, 
    TOO_FREQUENT_SERVICE_ACCESS, 
    UNEXPECTED_RADIO_LINK_FAILURES, 
    PING_PONG_ACROSS_CELLS
    };

    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const ExceptionId_anyOf& rhs) const;
    bool operator!=(const ExceptionId_anyOf& rhs) const;

    /////////////////////////////////////////////
    /// ExceptionId_anyOf members

    ExceptionId_anyOf::eExceptionId_anyOf getValue() const;
    void setValue(ExceptionId_anyOf::eExceptionId_anyOf value);
    
    friend void to_json(nlohmann::json& j, const ExceptionId_anyOf& o);
    friend void from_json(const nlohmann::json& j, ExceptionId_anyOf& o);
protected:
    ExceptionId_anyOf::eExceptionId_anyOf m_value = ExceptionId_anyOf::eExceptionId_anyOf::INVALID_VALUE_OPENAPI_GENERATED;
};

} // namespace oai::_3gpp::model

#endif /* ExceptionId_anyOf_H_ */
