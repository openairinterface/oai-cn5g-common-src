...
PduSessionModificationReject::PduSessionModificationReject()
    : ie_header_(k5gsSessionManagementMessages, kPduSessionModificationReject) {
...
int PduSessionModificationReject::Encode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Encoding PduSessionModificationReject message");
  int encoded_size    = 0;
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Encoded PduSessionModificationReject message len (%d)",
          encoded_size);
  return encoded_size;
}
...
int PduSessionModificationReject::Decode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Decoding PduSessionModificationReject message");
  int decoded_size    = 0;
  ...
  while ((octet != 0x0)) {
    switch (octet) {
      case kIeiGprsTimer3BackOffTimer: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiGprsTimer3BackOffTimer);
      ...
      case kIei5gsmCongestionReAttemptIndicator: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIei5gsmCongestionReAttemptIndicator);
        if ((decoded_ie_size = NasHelper::Decode(
      ...
      case kIeiReAttemptIndicator: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiReAttemptIndicator);
        if ((decoded_ie_size = NasHelper::Decode(
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Decoded PduSessionModificationReject message len (%d)",
          decoded_size);
  return decoded_size;
}
