...
bool PduSessionResourceSetupItemSUReq::encode(
  ...
  if (!m_SNssai.encode(pduSessionResourceItem.s_NSSAI))
    return false;
...
bool PduSessionResourceSetupItemSUReq::decode(
  ...
  if (!m_PduSessionId.decode(pduSessionResourceItem.pDUSessionID))
    return false;
  if (!m_SNssai.decode(pduSessionResourceItem.s_NSSAI))
    return false;

  if (pduSessionResourceItem.pDUSessionNAS_PDU) {
    NasPdu tmp = {};
    if (!tmp.decode(*pduSessionResourceItem.pDUSessionNAS_PDU))
      return false;
