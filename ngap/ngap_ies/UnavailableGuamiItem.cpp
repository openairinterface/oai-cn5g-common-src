...
//------------------------------------------------------------------------------
void UnavailableGuamiItem::setGuami(const Guami &guami) { m_Guami = guami; }

//------------------------------------------------------------------------------
void UnavailableGuamiItem::getGuami(Guami &guami) const { guami = m_Guami; }
...
bool UnavailableGuamiItem::encode(Ngap_UnavailableGUAMIItem &item) const {
  ...
  if (m_BackupAmfName.has_value())
    if (!m_BackupAmfName.value().encode(*item.backupAMFName))
      return false;
  return true;
}
...
bool UnavailableGuamiItem::decode(const Ngap_UnavailableGUAMIItem &item) {
  if (!m_Guami.decode(item.gUAMI))
    return false;

  if (item.timerApproachForGUAMIRemoval) {
    TimerApproachForGuamiRemoval tmp = {};
    if (!tmp.decode(*item.timerApproachForGUAMIRemoval))
      return false;
    m_TimerApproachForGuamiRemoval =
  ...
  if (item.backupAMFName) {
    AmfName tmp = {};
    if (!tmp.decode(*item.backupAMFName))
      return false;
