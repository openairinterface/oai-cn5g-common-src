...
int PduSessionEstablishmentReject::Encode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Encoding PduSessionEstablishmentReject message");
  int encoded_size    = 0;
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Encoded PduSessionEstablishmentReject message len (%d)",
          encoded_size);
  return encoded_size;
}
...
int PduSessionEstablishmentReject::Decode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Decoding PduSessionEstablishmentReject message");
  int decoded_size    = 0;
  ...
  while ((octet != 0x0)) {
    switch ((octet & 0xf0) >> 4) {
      case kIeiAllowedSscMode: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiAllowedSscMode);
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Decoded PduSessionEstablishmentReject message len (%d)",
          decoded_size);
  return decoded_size;
}
