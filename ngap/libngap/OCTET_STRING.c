...
void OCTET_STRING_free(const asn_TYPE_descriptor_t *td, void *sptr,
 ...
 */
int OCTET_STRING_compare(const asn_TYPE_descriptor_t *td, const void *aptr,
                         const void *bptr) {
...
int OCTET_STRING_per_put_characters(asn_per_outp_t *po, const uint8_t *buf,
  ...
  for (ub -= lb; buf < end; buf += bpc) {
    ...
    ch = value - lb;
    if (ch < 0 || ch > ub) {
      ASN_DEBUG("Character %d (0x%02x) is out of range (%ld..%ld)", *buf, value,
                lb, ub + lb);
      return -1;
    }
