...
asn_enc_rval_t OPEN_TYPE_uper_put(
  ...
  if (!elm->type->op || !elm->type->op->uper_encoder) {
    ASN_DEBUG(
        "Open Type %s->%s: UPER encoder is not defined", td->name, elm->name);
    ASN__ENCODE_FAILED;
  }
  ...
  ASN_DEBUG(
      "OPEN_TYPE_uper_put: elm->type=%s, elements=%p, elements_count=%u, "
  ...
  if (elm->type->elements_count > 0) {
    /*
     * CHOICE wrapper mode: encode the CHOICE directly without open type
     * wrapper. The CHOICE is already selected and structured, we just need to
     * encode it. For constrained encoding within a SEQUENCE (not in
     * extensions), we encode directly without the open type length determinant.
     */
