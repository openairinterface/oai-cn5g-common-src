...
bool UpPathChgEvent::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "UpPathChgEvent" : pathPrefix;

                
    if (!m_DnaiChgType.validate(msg, _pathPrefix + ".dnaiChgType")) {
        msg << _pathPrefix << ": DnaiChgType is invalid;";
        success = false;
    }    
    return success;
}
...
oai::model::DnaiChangeType UpPathChgEvent::getDnaiChgType() const
{
    return m_DnaiChgType;
}
void UpPathChgEvent::setDnaiChgType(oai::model::DnaiChangeType const& value)
{
    m_DnaiChgType = value;
}
