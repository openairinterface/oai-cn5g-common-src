...
asn_enc_rval_t OCTET_STRING_encode_xer(
...
static int OS__check_escaped_control_char(const void* buf, int size) {
  ...
  for (i = 0; i < 32 /* Don't spend time on the bottom half */; i++) {
    ...
    if (el->size == size && memcmp(buf, el->string, size) == 0) return i;
  }
  return -1;
}

static int OCTET_STRING__handle_control_chars(
...
asn_enc_rval_t OCTET_STRING_encode_xer_utf8(
    const asn_TYPE_descriptor_t* td, const void* sptr, int ilevel,
    enum xer_encoder_flags_e flags, asn_app_consume_bytes_f* cb,
    void* app_key) {
  const OCTET_STRING_t* st = (const OCTET_STRING_t*) sptr;
  ...
  encoded_len += (buf - ss);
  if ((buf - ss) && cb(ss, buf - ss, app_key) < 0) ASN__ENCODE_FAILED;
 ...
 */
static ssize_t OCTET_STRING__convert_hexadecimal(
    void* sptr, const void* chunk_buf, size_t chunk_size, int have_more) {
 ...
 */
static ssize_t OCTET_STRING__convert_binary(
    void* sptr, const void* chunk_buf, size_t chunk_size, int have_more) {
 ...
 * This is similar to OCTET_STRING__convert_hexadecimal but sets bits_unused to
 * 0.
 */
static ssize_t BIT_STRING__convert_hexadecimal(
    void* sptr, const void* chunk_buf, size_t chunk_size, int have_more) {
  BIT_STRING_t* st       = (BIT_STRING_t*) sptr;
  const char* chunk_stop = (const char*) chunk_buf;
  const char* p          = chunk_stop;
 ...
 */
static int BIT_STRING__is_binary(const void* chunk_buf, size_t chunk_size) {
...
/*
 * Auto-detect and convert from either binary or hexadecimal format for BIT
 * STRING. Supports explicit prefixes per X.693:
 *   - B'...' or b'...' for binary format
 *   - H'...' or h'...' for hexadecimal format
 * Without explicit prefixes, defaults to binary unless hexadecimal digits (2-9,
 * A-F) are found in the string.
 */
static ssize_t BIT_STRING__convert_binary_or_hex(
    void* sptr, const void* chunk_buf, size_t chunk_size, int have_more) {
  ...
  while (p < pend &&
         (*p == 0x09 || *p == 0x0a || *p == 0x0c || *p == 0x0d || *p == 0x20)) {
    p++;
  }
  ...
  if (explicit_binary || explicit_hex) {
    ...
    if (explicit_binary) {
      result = OCTET_STRING__convert_binary(
          sptr, content_start, content_size, have_more);
    } else {
      result = BIT_STRING__convert_hexadecimal(
          sptr, content_start, content_size, have_more);
    }

    if (result < 0) return result;

    /* Return total consumed from original buffer including prefix and closing
     * quote */
  ...
  if (BIT_STRING__is_binary(chunk_buf, chunk_size)) {
    ...
    return BIT_STRING__convert_hexadecimal(
        sptr, chunk_buf, chunk_size, have_more);
  }
}
 ...
 */
static int OS__strtoent(
    int base, const char* buf, const char* end, int32_t* ret_value) {
 ...
 */
static ssize_t OCTET_STRING__convert_entrefs(
    void* sptr, const void* chunk_buf, size_t chunk_size, int have_more) {
  OCTET_STRING_t* st = (OCTET_STRING_t*) sptr;
  const char* p      = (const char*) chunk_buf;
  const char* pend   = p + chunk_size;
 ...
 */
asn_enc_rval_t OCTET_STRING_encode_xer_base64(
    const asn_TYPE_descriptor_t* td, const void* sptr, int ilevel,
    enum xer_encoder_flags_e flags, asn_app_consume_bytes_f* cb,
    void* app_key) {
  const OCTET_STRING_t* st = (const OCTET_STRING_t*) sptr;
 ...
 */
static ssize_t OCTET_STRING__convert_base64(
    void* sptr, const void* chunk_buf, size_t chunk_size, int have_more) {
  OCTET_STRING_t* st = (OCTET_STRING_t*) sptr;
  const char* p      = (const char*) chunk_buf;
  const char* pend   = p + chunk_size;
 ...
 */
static int OCTET_STRING__is_hexadecimal(
 ...
 */
static ssize_t OCTET_STRING__convert_auto(
    void* sptr, const void* chunk_buf, size_t chunk_size, int have_more) {
  const unsigned char* buf_start = (const unsigned char*) chunk_buf;
  const unsigned char* p         = buf_start;
  const unsigned char* pend      = p + chunk_size;
  ...
  if ((p + 1) < pend && (*p == 'H' || *p == 'h') && p[1] == '\'') {
    ...
    result =
        OCTET_STRING__convert_hexadecimal(sptr, content_start, content_size, 0);
    if (result < 0) return result;

    /* Return total consumed from original buffer including prefix and closing
     * quote */
    size_t total_consumed =
        (content_end - buf_start) + 1; /* +1 for closing quote */
    return total_consumed;
  }

  /* No explicit prefix - auto-detect based on content */
  if (OCTET_STRING__is_hexadecimal(chunk_buf, chunk_size)) {
    return OCTET_STRING__convert_hexadecimal(
        sptr, chunk_buf, chunk_size, have_more);
  } else {
...
static asn_dec_rval_t OCTET_STRING__decode_xer(
  ...
  return xer_decode_general(
      opt_codec_ctx, ctx, *sptr, xml_tag, buf_ptr, size,
 ...
 */
asn_dec_rval_t OCTET_STRING_decode_xer_hex(
    const asn_codec_ctx_t* opt_codec_ctx, const asn_TYPE_descriptor_t* td,
    void** sptr, const char* opt_mname, const void* buf_ptr, size_t size) {
  return OCTET_STRING__decode_xer(
      opt_codec_ctx, td, sptr, opt_mname, buf_ptr, size, 0,
 ...
 */
asn_dec_rval_t OCTET_STRING_decode_xer_binary(
    const asn_codec_ctx_t* opt_codec_ctx, const asn_TYPE_descriptor_t* td,
    void** sptr, const char* opt_mname, const void* buf_ptr, size_t size) {
  return OCTET_STRING__decode_xer(
      opt_codec_ctx, td, sptr, opt_mname, buf_ptr, size, 0,
 ...
 */
asn_dec_rval_t BIT_STRING_decode_xer_binary_or_hex(
    const asn_codec_ctx_t* opt_codec_ctx, const asn_TYPE_descriptor_t* td,
    void** sptr, const char* opt_mname, const void* buf_ptr, size_t size) {
  return OCTET_STRING__decode_xer(
      opt_codec_ctx, td, sptr, opt_mname, buf_ptr, size, 0,
...
asn_dec_rval_t OCTET_STRING_decode_xer_utf8(
    ...
    void** sptr, const char* opt_mname, const void* buf_ptr, size_t size) {
  return OCTET_STRING__decode_xer(
      opt_codec_ctx, td, sptr, opt_mname, buf_ptr, size,
      OCTET_STRING__handle_control_chars, OCTET_STRING__convert_entrefs);
}

/*
 * Decode OCTET STRING from Base64-encoded data.
 */
asn_dec_rval_t OCTET_STRING_decode_xer_base64(
    const asn_codec_ctx_t* opt_codec_ctx, const asn_TYPE_descriptor_t* td,
    void** sptr, const char* opt_mname, const void* buf_ptr, size_t size) {
  return OCTET_STRING__decode_xer(
      opt_codec_ctx, td, sptr, opt_mname, buf_ptr, size, 0,
 ...
 */
asn_dec_rval_t OCTET_STRING_decode_xer_auto(
    const asn_codec_ctx_t* opt_codec_ctx, const asn_TYPE_descriptor_t* td,
    void** sptr, const char* opt_mname, const void* buf_ptr, size_t size) {
  return OCTET_STRING__decode_xer(
      opt_codec_ctx, td, sptr, opt_mname, buf_ptr, size, 0,
