...
asn_dec_rval_t INTEGER_decode_uper(
  ...
  if (ct && ct->flags != APC_UNCONSTRAINED) {
    ...
    if (ct->range_bits >= 0) {
      if ((size_t) ct->range_bits > 8 * sizeof(uintmax_t)) ASN__DECODE_FAILED;
      ...
      if (specs && specs->field_unsigned) {
        ...
        if (uper_get_constrained_whole_number(pd, &uvalue, ct->range_bits))
          ASN__DECODE_STARVED;
        ASN_DEBUG(
            "Got value %" ASN_PRIuMAX " + low %" ASN_PRIdMAX "", uvalue,
            ct->lower_bound);
        uvalue += ct->lower_bound;
        if (uvalue > (uintmax_t) ct->upper_bound) ASN__DECODE_FAILED;
        if (asn_umax2INTEGER(st, uvalue)) ASN__DECODE_FAILED;
      } else {
        uintmax_t uvalue = 0;
        intmax_t svalue;
        if (uper_get_constrained_whole_number(pd, &uvalue, ct->range_bits))
        ...
        ASN_DEBUG(
            "Got value %" ASN_PRIuMAX " + low %" ASN_PRIdMAX "", uvalue,
            ct->lower_bound);
        if (per_imax_range_unrebase(
                uvalue, ct->lower_bound, ct->upper_bound, &svalue) ||
  ...
  if (opt_codec_ctx && opt_codec_ctx->uper_canonical && !ct && st->size > 1) {
    ...
    if (st->buf[0] == 0x00 && (st->buf[1] & 0x80) == 0) {
      ...
      if (opt_codec_ctx->uper_canonical_lenient) {
        ASN_DEBUG(
            "Non-canonical UPER: leading zeros in positive integer (lenient "
            "mode - continuing)");
      } else {
      ...
      if (opt_codec_ctx->uper_canonical_lenient) {
        ASN_DEBUG(
            "Non-canonical UPER: leading ones in negative integer (lenient "
            "mode - continuing)");
      } else {
  ...
  if (ct && ct->lower_bound) {
    ...
    if (asn_imax2INTEGER(st, value + ct->lower_bound)) ASN__DECODE_FAILED;
  }

  return rval;
}

asn_enc_rval_t INTEGER_encode_uper(
  ...
  if (ct) {
    int inext = 0;
    if (specs && specs->field_unsigned) {
      if (asn_INTEGER2umax(st, &value.u)) ASN__ENCODE_FAILED;
      /* Check proper range */
      if (ct->flags & APC_SEMI_CONSTRAINED) {
        if (value.u < (uintmax_t) ct->lower_bound) inext = 1;
      ...
      ASN_DEBUG(
          "Value %" ASN_PRIuMAX " (%02x/%" ASN_PRI_SIZE ") lb %" ASN_PRIuMAX
          " ub %" ASN_PRIuMAX " %s",
          value.u, st->buf[0], st->size, (uintmax_t) ct->lower_bound,
          (uintmax_t) ct->upper_bound, inext ? "ext" : "fix");
    } else {
      if (asn_INTEGER2imax(st, &value.s)) ASN__ENCODE_FAILED;
      /* Check proper range */
      if (ct->flags & APC_SEMI_CONSTRAINED) {
        if (value.s < ct->lower_bound) inext = 1;
      } else if (ct->range_bits >= 0) {
        if (value.s < ct->lower_bound || value.s > ct->upper_bound) inext = 1;
      }
      ASN_DEBUG(
          "Value %" ASN_PRIdMAX " (%02x/%" ASN_PRI_SIZE ") lb %" ASN_PRIdMAX
          " ub %" ASN_PRIdMAX " %s",
          value.s, st->buf[0], st->size, ct->lower_bound, ct->upper_bound,
          inext ? "ext" : "fix");
    }
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    if (specs && specs->field_unsigned) {
      if (((uintmax_t) ct->lower_bound > (uintmax_t) (ct->upper_bound) ||
        ...
        ASN_DEBUG(
            "Value %" ASN_PRIuMAX
            " to-be-encoded is outside the bounds [%" ASN_PRIuMAX
            ", %" ASN_PRIuMAX "]!",
      ...
      if (per_imax_range_rebase(
              value.s, ct->lower_bound, ct->upper_bound, &v)) {
        ASN__ENCODE_FAILED;
      }
    }
    ASN_DEBUG(
        "Encoding integer %" ASN_PRIuMAX " with range %d bits", v,
        ct->range_bits);
