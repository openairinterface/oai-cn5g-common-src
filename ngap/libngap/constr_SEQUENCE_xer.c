 ...
 */
asn_dec_rval_t SEQUENCE_decode_xer(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  for (edx = ctx->step; ctx->phase <= 3;) {
    ...
    if (ctx->phase == 2) {
      ...
      if (elm->flags & ATF_OPEN_TYPE) {
        ...
        tmprval = elm->type->op->xer_decoder(opt_codec_ctx, elm->type,
                                             memb_ptr2, elm->name, ptr, size);
      }
    ...
    case XCT_UNKNOWN_BO:
      ...
      ASN_DEBUG("XER/SEQUENCE: tcv=%d, ph=%d, edx=%" ASN_PRI_SIZE "", tcv,
                ctx->phase, edx);
      if (ctx->phase != 1) {
      ...
      if (edx < td->elements_count) {
        ...
        ASN_DEBUG("Out of defined members: %" ASN_PRI_SIZE "/%u", edx,
                  td->elements_count);
      }
      ...
      if (IN_EXTENSION_GROUP(
              specs,
              edx + (edx < td->elements_count ? elements[edx].optional : 0))) {
...
asn_enc_rval_t SEQUENCE_encode_xer(const asn_TYPE_descriptor_t *td,
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    tmper = elm->type->op->xer_encoder(elm->type, memb_ptr, ilevel + 1, flags,
                                       cb, app_key);
    if (tmp_def_val) {
