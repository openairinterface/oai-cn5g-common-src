...
asn_dec_rval_t OPEN_TYPE_aper_get(
  ...
  if (!elm->type) {
    ASN_DEBUG("Open Type %s->%s: type descriptor is NULL", td->name, elm->name);
    ASN__DECODE_FAILED;
  }
  ...
  if (!selected.presence_index || !selected.type_descriptor) {
    ASN_DEBUG(
        "Open Type %s->%s: selected type descriptor is NULL", td->name,
        elm->name);
    ASN__DECODE_FAILED;
  }

  ASN_DEBUG(
      "OPEN_TYPE_aper_get: elm->type=%s, elements=%p, elements_count=%u, "
      "selected.presence_index=%u, selected.type=%s",
  ...
  /* Check if this OPEN_TYPE uses CHOICE wrapper (elements_count > 0) or direct
   * type */
  if (elm->type->elements_count > 0) {
    ...
    if (selected.presence_index > elm->type->elements_count) {
      ASN_DEBUG(
          "Open Type %s->%s: presence index %u out of bounds (max %u)",
    ...
    if (!elm->type->elements) {
      ASN_DEBUG(
          "Open Type %s->%s: elements array is NULL but elements_count is %u",
    ...
    if (*memb_ptr2 == NULL) {
      ...
      if (!specs) {
        ASN_DEBUG(
            "Open Type %s->%s: type specifics is NULL", td->name, elm->name);
        ASN__DECODE_FAILED;
      }
    ...
    ASN_DEBUG(
        "Open Type %s->%s: using direct type mode (no CHOICE wrapper)",
        td->name, elm->name);
  }

  /* Compute target pointer and constraints based on CHOICE wrapper mode or
   * direct type mode */
  ...
  rv = aper_open_type_get(
      opt_codec_ctx, selected.type_descriptor, constraints, &inner_value, pd);
  ASN_DEBUG(
      "aper_open_type_get returned code=%d for %s", rv.code,
  ...
  switch (rv.code) {
    case RC_OK:
      if (elm->type->elements_count > 0) {
        ...
        ASN_DEBUG(
            "Calling CHOICE_variant_set_presence(elm->type=%s, "
            "presence_index=%u, elements_count=%u)",
            elm->type->name, selected.presence_index,
            elm->type->elements_count);
        if (CHOICE_variant_set_presence(
                elm->type, *memb_ptr2, selected.presence_index) == 0) {
          /* CHOICE wrapper mode: for pointer variants, copy decoded pointer
           * back to field */
          if (variant_elm && (variant_elm->flags & ATF_POINTER)) {
            /*
             * The decoder allocated a structure and stored pointer in
             * inner_value. Copy it back to the actual field in the CHOICE
             * structure.
             */
        ...
        /* Direct type mode: update member pointer with decoded value if pointer
         * type */
        if (elm->flags & ATF_POINTER) {
    ...
    case RC_FAIL:
      ...
      if (elm->type->elements_count > 0) {
        ...
        if (*memb_ptr2) CHOICE_variant_set_presence(elm->type, *memb_ptr2, 0);
...
asn_enc_rval_t OPEN_TYPE_aper_put(
  ...
  if (!elm->type->op || !elm->type->op->aper_encoder) {
    ASN_DEBUG(
        "Open Type %s->%s: APER encoder is not defined", td->name, elm->name);
    ASN__ENCODE_FAILED;
  }
  ...
  if (!selected.type_descriptor) {
    ASN_DEBUG(
        "Open Type %s->%s: type_selector returned NULL type descriptor",
        td->name, elm->name);
    ASN__ENCODE_FAILED;
  }

  ASN_DEBUG(
      "OPEN_TYPE_aper_put: elm->type=%s, elements=%p, elements_count=%u, "
      "selected.presence_index=%u, selected.type=%s",
      elm->type->name, (void*) elm->type->elements, elm->type->elements_count,
      selected.presence_index, selected.type_descriptor->name);
  ...
  if (elm->type->elements_count > 0) {
     ...
     * APER encoder. The encoder will handle the choice index and variant
     * encoding according to its own constraints (constrained vs. extension
     * variants). Wrapping it again with open type framing would be redundant
     * and incorrect.
     */
     ...
     */
    ASN_DEBUG(
        "Direct type mode: encoding using %s wrapped in OPEN TYPE",
        selected.type_descriptor->name);
...
int OPEN_TYPE_aper_is_unknown_type(
    const asn_TYPE_descriptor_t* td, void* sptr, const asn_TYPE_member_t* elm) {
  asn_type_selector_result_t selected;
...
#define ASN_DUMMY_BYTES 256
  ...
  do {
    ...
    if (bytes > 10 * ASN_DUMMY_BYTES) {
      ...
      if (!dummy_ptr) return rv;
    }

    if (per_get_many_bits(pd, (dummy_ptr ? dummy_ptr : dummy), 0, bytes << 3) <
        0) {
