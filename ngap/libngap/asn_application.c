 ...
 */
static int callback_failure_catch_cb(const void *data, size_t size,
                                     void *keyp) {
...
asn_enc_rval_t asn_encode_to_buffer(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  er = asn_encode_internal(opt_codec_ctx, syntax, td, sptr, overrun_encoder_cb,
                           &buf_key);
...
static asn_enc_rval_t asn_encode_internal(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  case ATS_NONSTANDARD_PLAINTEXT:
    if (td->op->print_struct) {
      ...
      if (td->op->print_struct(td, sptr, 1, callback_count_bytes_cb, &cb_key) <
              0 ||
