/**
* Nnef_EventExposure
* NEF Event Exposure Service.   © 2022 , 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.0
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#include "DynamicPolicy.h"
#include "Helpers.h"

#include <sstream>

namespace oai::_3gpp::model
{

DynamicPolicy::DynamicPolicy()
{
    m_PolicyTemplateId = "";
    m_MediaTypeIsSet = false;
    m_ProvisioningSessionId = "";
    m_QosSpecificationIsSet = false;
    m_EnforcementMethod = "";
    m_EnforcementMethodIsSet = false;
    m_EnforcementBitRate = 0;
    m_EnforcementBitRateIsSet = false;
    
}

void DynamicPolicy::validate() const
{
    std::stringstream msg;
    if (!validate(msg))
    {
        throw oai::_3gpp::model::helpers::ValidationException(msg.str());
    }
}

bool DynamicPolicy::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool DynamicPolicy::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "DynamicPolicy" : pathPrefix;

                 
    
    /* ServiceDataFlowDescriptions */ {
        const std::vector<oai::_3gpp::model::ServiceDataFlowDescription>& value = m_ServiceDataFlowDescriptions;
        const std::string currentValuePath = _pathPrefix + ".serviceDataFlowDescriptions";
                
        
        { // Recursive validation of array elements
            const std::string oldValuePath = currentValuePath;
            int i = 0;
            for (const oai::_3gpp::model::ServiceDataFlowDescription& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".serviceDataFlowDescriptions") && success;
 
                i++;
            }
        }

    }
                        
    return success;
}

bool DynamicPolicy::operator==(const DynamicPolicy& rhs) const
{
    return
    
    
    (getDynamicPolicyId() == rhs.getDynamicPolicyId())
     &&
    
    (getPolicyTemplateId() == rhs.getPolicyTemplateId())
     &&
    
    (getServiceDataFlowDescriptions() == rhs.getServiceDataFlowDescriptions())
     &&
    
    
    ((!mediaTypeIsSet() && !rhs.mediaTypeIsSet()) || (mediaTypeIsSet() && rhs.mediaTypeIsSet() && getMediaType() == rhs.getMediaType())) &&
    
    (getProvisioningSessionId() == rhs.getProvisioningSessionId())
     &&
    
    
    ((!qosSpecificationIsSet() && !rhs.qosSpecificationIsSet()) || (qosSpecificationIsSet() && rhs.qosSpecificationIsSet() && getQosSpecification() == rhs.getQosSpecification())) &&
    
    
    ((!enforcementMethodIsSet() && !rhs.enforcementMethodIsSet()) || (enforcementMethodIsSet() && rhs.enforcementMethodIsSet() && getEnforcementMethod() == rhs.getEnforcementMethod())) &&
    
    
    ((!enforcementBitRateIsSet() && !rhs.enforcementBitRateIsSet()) || (enforcementBitRateIsSet() && rhs.enforcementBitRateIsSet() && getEnforcementBitRate() == rhs.getEnforcementBitRate()))
    
    ;
}

bool DynamicPolicy::operator!=(const DynamicPolicy& rhs) const
{
    return !(*this == rhs);
}

void to_json(nlohmann::json& j, const DynamicPolicy& o)
{
    j = nlohmann::json();
    j["dynamicPolicyId"] = o.m_DynamicPolicyId;
    j["policyTemplateId"] = o.m_PolicyTemplateId;
    j["serviceDataFlowDescriptions"] = o.m_ServiceDataFlowDescriptions;
    if(o.mediaTypeIsSet())
        j["mediaType"] = o.m_MediaType;
    j["provisioningSessionId"] = o.m_ProvisioningSessionId;
    if(o.qosSpecificationIsSet())
        j["qosSpecification"] = o.m_QosSpecification;
    if(o.enforcementMethodIsSet())
        j["enforcementMethod"] = o.m_EnforcementMethod;
    if(o.enforcementBitRateIsSet())
        j["enforcementBitRate"] = o.m_EnforcementBitRate;
    
}

void from_json(const nlohmann::json& j, DynamicPolicy& o)
{
    j.at("dynamicPolicyId").get_to(o.m_DynamicPolicyId);
    j.at("policyTemplateId").get_to(o.m_PolicyTemplateId);
    j.at("serviceDataFlowDescriptions").get_to(o.m_ServiceDataFlowDescriptions);
    if(j.find("mediaType") != j.end())
    {
        j.at("mediaType").get_to(o.m_MediaType);
        o.m_MediaTypeIsSet = true;
    } 
    j.at("provisioningSessionId").get_to(o.m_ProvisioningSessionId);
    if(j.find("qosSpecification") != j.end())
    {
        j.at("qosSpecification").get_to(o.m_QosSpecification);
        o.m_QosSpecificationIsSet = true;
    } 
    if(j.find("enforcementMethod") != j.end())
    {
        j.at("enforcementMethod").get_to(o.m_EnforcementMethod);
        o.m_EnforcementMethodIsSet = true;
    } 
    if(j.find("enforcementBitRate") != j.end())
    {
        j.at("enforcementBitRate").get_to(o.m_EnforcementBitRate);
        o.m_EnforcementBitRateIsSet = true;
    } 
    
}

oai::_3gpp::model::std::string DynamicPolicy::getDynamicPolicyId() const
{
    return m_DynamicPolicyId;
}
void DynamicPolicy::setDynamicPolicyId(oai::_3gpp::model::std::string const& value)
{
    m_DynamicPolicyId = value;
}
std::string DynamicPolicy::getPolicyTemplateId() const
{
    return m_PolicyTemplateId;
}
void DynamicPolicy::setPolicyTemplateId(std::string const& value)
{
    m_PolicyTemplateId = value;
}
std::vector<oai::_3gpp::model::ServiceDataFlowDescription> DynamicPolicy::getServiceDataFlowDescriptions() const
{
    return m_ServiceDataFlowDescriptions;
}
void DynamicPolicy::setServiceDataFlowDescriptions(std::vector<oai::_3gpp::model::ServiceDataFlowDescription> const& value)
{
    m_ServiceDataFlowDescriptions = value;
}
oai::_3gpp::model::MediaType DynamicPolicy::getMediaType() const
{
    return m_MediaType;
}
void DynamicPolicy::setMediaType(oai::_3gpp::model::MediaType const& value)
{
    m_MediaType = value;
    m_MediaTypeIsSet = true;
}
bool DynamicPolicy::mediaTypeIsSet() const
{
    return m_MediaTypeIsSet;
}
void DynamicPolicy::unsetMediaType()
{
    m_MediaTypeIsSet = false;
}
std::string DynamicPolicy::getProvisioningSessionId() const
{
    return m_ProvisioningSessionId;
}
void DynamicPolicy::setProvisioningSessionId(std::string const& value)
{
    m_ProvisioningSessionId = value;
}
oai::_3gpp::model::M5QoSSpecification DynamicPolicy::getQosSpecification() const
{
    return m_QosSpecification;
}
void DynamicPolicy::setQosSpecification(oai::_3gpp::model::M5QoSSpecification const& value)
{
    m_QosSpecification = value;
    m_QosSpecificationIsSet = true;
}
bool DynamicPolicy::qosSpecificationIsSet() const
{
    return m_QosSpecificationIsSet;
}
void DynamicPolicy::unsetQosSpecification()
{
    m_QosSpecificationIsSet = false;
}
std::string DynamicPolicy::getEnforcementMethod() const
{
    return m_EnforcementMethod;
}
void DynamicPolicy::setEnforcementMethod(std::string const& value)
{
    m_EnforcementMethod = value;
    m_EnforcementMethodIsSet = true;
}
bool DynamicPolicy::enforcementMethodIsSet() const
{
    return m_EnforcementMethodIsSet;
}
void DynamicPolicy::unsetEnforcementMethod()
{
    m_EnforcementMethodIsSet = false;
}
int32_t DynamicPolicy::getEnforcementBitRate() const
{
    return m_EnforcementBitRate;
}
void DynamicPolicy::setEnforcementBitRate(int32_t const value)
{
    m_EnforcementBitRate = value;
    m_EnforcementBitRateIsSet = true;
}
bool DynamicPolicy::enforcementBitRateIsSet() const
{
    return m_EnforcementBitRateIsSet;
}
void DynamicPolicy::unsetEnforcementBitRate()
{
    m_EnforcementBitRateIsSet = false;
}


} // namespace oai::_3gpp::model

