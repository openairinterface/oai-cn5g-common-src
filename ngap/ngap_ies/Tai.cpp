...
//------------------------------------------------------------------------------
void Tai::set(const std::string &mcc, const std::string &mnc,
              const uint32_t &tac) {
...
bool Tai::encode(Ngap_TAI_t &tai) const {
  if (!m_PlmnId.encode(tai.pLMNIdentity))
    return false;
  if (!m_Tac.encode(tai.tAC))
    return false;
...
bool Tai::decode(const Ngap_TAI_t &tai) {
  if (!m_PlmnId.decode(tai.pLMNIdentity))
    return false;
  if (!m_Tac.decode(tai.tAC))
    return false;
