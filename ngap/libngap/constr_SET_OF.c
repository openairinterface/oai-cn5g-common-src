...
#if !defined(ASN_DISABLE_UPER_SUPPORT)
    case SOES_CUPER:
      erval = uper_encode(elm->type, elm->encoding_constraints.per_constraints,
...
#if !defined(ASN_DISABLE_APER_SUPPORT)
    case SOES_CAPER:
      erval = aper_encode(elm->type, elm->encoding_constraints.per_constraints,
                          memb_ptr, _el_addbytes, encoding_el);
      if (erval.encoded != -1) {
        size_t extra_bits = erval.encoded % 8;
        assert(encoding_el->length == (size_t)(erval.encoded + 7) / 8);
...
void SET_OF_free(const asn_TYPE_descriptor_t *td, void *ptr,
...
int SET_OF_constraint(const asn_TYPE_descriptor_t *td, const void *sptr,
...
int SET_OF_compare(const asn_TYPE_descriptor_t *td, const void *aptr,
                   const void *bptr) {
