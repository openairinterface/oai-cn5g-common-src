...
asn_dec_rval_t SEQUENCE_decode_aper(
  ...
  if (extpresent) {
    ...
    epmd.nbits  = bmlength;
    ASN_DEBUG(
        "Read in extensions bitmap for %s of %zd bits (%x..)", td->name,
        bmlength, *epres);
...
static int SEQUENCE_handle_extensions_aper(
  ...
  for (edx = specs->first_extension; edx < td->elements_count; edx++) {
    ...
    ASN_DEBUG(
        "checking %s (@%zu) present => %d", elm->type->name, edx, present);
    exts_count++;
...
asn_enc_rval_t SEQUENCE_encode_aper(
