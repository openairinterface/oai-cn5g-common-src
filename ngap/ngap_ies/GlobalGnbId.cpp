...
bool GlobalGnbId::encode(Ngap_GlobalGNB_ID_t &globalGnbId) const {
  if (!m_PlmnId.encode(globalGnbId.pLMNIdentity))
    return false;
  if (!m_GnbId.encode(globalGnbId.gNB_ID))
    return false;
...
bool GlobalGnbId::decode(const Ngap_GlobalGNB_ID_t &globalGnbId) {
  if (!m_PlmnId.decode(globalGnbId.pLMNIdentity))
    return false;
  if (!m_GnbId.decode(globalGnbId.gNB_ID))
    return false;
