...
bool EventForMdt_anyOf::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "EventForMdt_anyOf" : pathPrefix;

    
    if (m_value == EventForMdt_anyOf::eEventForMdt_anyOf::INVALID_VALUE_OPENAPI_GENERATED)
    {
        success = false;
