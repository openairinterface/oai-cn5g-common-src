...
//------------------------------------------------------------------------------
int oai::utils::get_inet_addr_infos_from_iface(const std::string &if_name,
  ...
  if (ioctl(fd, SIOCGIFADDR, &ifr)) {
    close(fd);
    logger_common::system().error("Failed to probe %s inet addr: error %s\n",
  ...
  if (ioctl(fd, SIOCGIFNETMASK, &ifr)) {
    close(fd);
    logger_common::system().error("Failed to probe %s inet netmask: error %s\n",
                                  if_name.c_str(), strerror(errno));
    return RETURNerror;
  }
  ...
  if (ioctl(fd, SIOCGIFMTU, &ifr)) {
    logger_common::system().error("Failed to probe %s MTU: error %s\n",
                                  if_name.c_str(), strerror(errno));
  } else {
    mtu = ifr.ifr_mtu;
  }
