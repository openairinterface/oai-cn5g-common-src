...
function usage {
    echo "OAI GitHub PR validation script (Signed-off-by and merge commits)"
    echo ""
    echo "Usage:"
    echo "------"
    echo "    $0 -s <source-ref> -t <target-branch>"
    echo ""
    echo "Options:"
    echo "--------"
    echo "    -s"
    echo "    The source branch or commit of the pull request. Default value is current Git Branch (HEAD)"
    echo ""
    echo "    -t"
