...
int PduSessionEstablishmentRequest::Encode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Encoding PduSessionEstablishmentRequest message");
  int encoded_size    = 0;
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Encoded PduSessionEstablishmentRequest message len (%d)",
          encoded_size);
  return encoded_size;
}
...
int PduSessionEstablishmentRequest::Decode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Decoding PduSessionEstablishmentRequest message");
  int decoded_size    = 0;
  ...
  while ((octet != 0x0)) {
    switch ((octet & 0xf0) >> 4) {
      case kIeiPduSessionType: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiPduSessionType);
        if ((decoded_ie_size = NasHelper::Decode(
      ...
      case kIeiSscMode: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiSscMode);
        if ((decoded_ie_size = NasHelper::Decode(
    ...
    switch (octet) {
      ...
      case kIeiSmPduDnRequestContainer: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiSmPduDnRequestContainer);
        if ((decoded_ie_size = NasHelper::Decode(
      ...
      case kIeiIpHeaderCompressionConfiguration: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiIpHeaderCompressionConfiguration);
        if ((decoded_ie_size = NasHelper::Decode(
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Decoded PduSessionEstablishmentRequest message len (%d)",
          decoded_size);
  return decoded_size;
}
