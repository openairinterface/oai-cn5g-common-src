...
asn_dec_rval_t SET_OF_decode_uper(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  if (ct && ct->effective_bits >= 0) {
    ...
    ASN_DEBUG("Preparing to fetch %ld+%ld elements from %s", (long)nelems,
  ...
  do {
    ...
    if (nelems < 0) {
      ...
      ASN_DEBUG("Got to decode %" ASN_PRI_SSIZE " elements (eff %d)", nelems,
...
asn_enc_rval_t SET_OF_encode_uper(const asn_TYPE_descriptor_t *td,
                                  const asn_per_constraints_t *constraints,
