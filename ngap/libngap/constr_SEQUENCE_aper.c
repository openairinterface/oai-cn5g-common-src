...
asn_dec_rval_t SEQUENCE_decode_aper(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  if (extpresent) {
    ...
    ASN_DEBUG("Extensions %" ASN_PRI_SSIZE " present in %s", bmlength,
              td->name);
    ...
    epmd.buffer = epres;
    epmd.nbits = bmlength;
    ASN_DEBUG("Read in extensions bitmap for %s of %zd bits (%x..)", td->name,
              bmlength, *epres);
...
static int SEQUENCE_handle_extensions_aper(const asn_TYPE_descriptor_t *td,
                                           const void *sptr,
                                           asn_per_outp_t *po1,
                                           asn_per_outp_t *po2) {
  const asn_SEQUENCE_specifics_t *specs =
  ...
  for (edx = specs->first_extension; edx < td->elements_count; edx++) {
    ...
    ASN_DEBUG("checking %s (@%zu) present => %d", elm->type->name, edx,
              present);
    exts_count++;
    ...
    if (po2 && present &&
        aper_open_type_put(elm->type, elm->encoding_constraints.per_constraints,
                           *memb_ptr2, po2))
      return -1;
  }
...
asn_enc_rval_t SEQUENCE_encode_aper(const asn_TYPE_descriptor_t *td,
                                    const asn_per_constraints_t *constraints,
  ...
  for (edx = 0;
    ...
    if (elm->flags & ATF_POINTER) {
      memb_ptr2 = (const void *const *)((const char *)sptr + elm->memb_offset);
      if (!*memb_ptr2) {
        ASN_DEBUG("Element %s %zu not present", elm->name, edx);
        if (elm->optional)
          continue;
