...
public:
  ...
  void SetSuciWithSupiImsi(const std::string &mcc, const std::string &mnc,
                           const std::string &routing_ind,
                           uint8_t protection_sch_id, uint8_t home_pki,
                           const std::string &msin_digits);
  bool GetSuciWithSupiImsi(SUCI_imsi_t &suci) const;

  int EncodeRoutingIndicator(std::optional<std::string> routing_indicator,
  ...
  void Set5gSTmsi(uint16_t amf_set_id, uint8_t amf_pointer,
                  const std::string &tmsi);
  bool Get5gSTmsi(uint16_t &amf_set_id, uint8_t &amf_pointer,
