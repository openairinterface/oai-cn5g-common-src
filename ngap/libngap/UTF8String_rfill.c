...
static size_t UTF8String__encode_codepoint(uint8_t* b, uint32_t code) {
  if (code <= 0x7f) {
    b[0] = code;
    return 1;
  } else if (code <= 0x7ff) {
...
static const asn_per_constraints_t* UTF8String__per_constraints(
...
asn_random_fill_result_t UTF8String_random_fill(
  ...
  if (pc) {
    if ((pc->value.flags & APC_CONSTRAINED) && pc->value.lower_bound >= 0 &&
     ...
     */
    if (pc->size.flags & APC_CONSTRAINED) force_single_octets = 1;
  }
  ...
  if (has_value_constraint && force_single_octets) {
    if (value_lb > 0x7f) {
      ...
      if (value_ub > 0x7f) value_ub = 0x7f;
    }
  }
