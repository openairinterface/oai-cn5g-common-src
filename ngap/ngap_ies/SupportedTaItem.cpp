...
//------------------------------------------------------------------------------
void SupportedTaItem::setTac(const TAC &m_tac) { m_Tac = m_tac; }

//------------------------------------------------------------------------------
void SupportedTaItem::getTac(TAC &m_tac) const { m_tac = m_Tac; }

//------------------------------------------------------------------------------
TAC SupportedTaItem::getTac() const { return m_Tac; }
...
bool SupportedTaItem::decode(const Ngap_SupportedTAItem_t &pdu) {
  if (!m_Tac.decode(pdu.tAC))
    return false;
  for (int i = 0; i < pdu.broadcastPLMNList.list.count; i++) {
    BroadcastPlmnItem item = {};
    if (!item.decode(*pdu.broadcastPLMNList.list.array[i]))
      return false;
