...
void config::to_json(nlohmann::json& json_data) {
  ...
  json_data[m_tls.get_config_name()] = m_tls.to_json();
...
bool config::from_json(const nlohmann::json& json_data) {
  try {
    ...
    if (json_data.find(m_tls.get_config_name()) != json_data.end()) {
      m_tls.from_json(json_data[m_tls.get_config_name()]);
    }
...
std::string config::to_string() const {
  ...
  out.append(m_tls.to_string(indent));
...
uint32_t config::get_http_request_timeout() const {
