...
public:
  ...
  void setUeAggregateMaximumBitRate(const long &bitRateDl,
                                    const long &bitRateUl);
  // TODO: getUeAggregateMaximumBitRate

  void setUeSecurityCapabilities(uint16_t nrEncryptionAlgs,
                                 uint16_t nrIntegrityProtectionAlgs,
                                 uint16_t eutraEncryptionAlgs,
                                 uint16_t eutraIntegrityProtectionAlgs);
  ...
  void
  setSourceToTargetTransparentContainer(const OCTET_STRING_t &sourceTotarget);
  // TODO: getSourceToTargetTransparentContainer

  void setAllowedNssai(const std::vector<SNssai> &list);
  // TODO: getAllowedNssai

  void setGuami(const PlmnId &plmnId, const AmfRegionId &amfRegionId,
                const AmfSetId &amfSetId, const AmfPointer &amfPointer);
  void setGuami(const std::string &mcc, const std::string &mnc,
