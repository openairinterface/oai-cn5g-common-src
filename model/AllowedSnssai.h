/**
* NSSF NS Selection
* NSSF Network Slice Selection Service.   © 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 2.2.2
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * AllowedSnssai.h
 *
 * Contains the authorized S-NSSAI and optional mapped home S-NSSAI and network slice instance information 
 */

#ifndef AllowedSnssai_H_
#define AllowedSnssai_H_


#include "NsiInformation.h"
#include "Snssai.h"
#include <vector>
#include <nlohmann/json.hpp>

namespace oai::model
{

/// <summary>
/// Contains the authorized S-NSSAI and optional mapped home S-NSSAI and network slice instance information 
/// </summary>
class  AllowedSnssai
{
public:
    AllowedSnssai();
    virtual ~AllowedSnssai() = default;


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const AllowedSnssai& rhs) const;
    bool operator!=(const AllowedSnssai& rhs) const;

    /////////////////////////////////////////////
    /// AllowedSnssai members

    /// <summary>
    /// 
    /// </summary>
    oai::model::Snssai getAllowedSnssai() const;
    void setAllowedSnssai(oai::model::Snssai const& value);
    /// <summary>
    /// 
    /// </summary>
    std::vector<oai::model::NsiInformation> getNsiInformationList() const;
    void setNsiInformationList(std::vector<oai::model::NsiInformation> const& value);
    bool nsiInformationListIsSet() const;
    void unsetNsiInformationList();
    /// <summary>
    /// 
    /// </summary>
    oai::model::Snssai getMappedHomeSnssai() const;
    void setMappedHomeSnssai(oai::model::Snssai const& value);
    bool mappedHomeSnssaiIsSet() const;
    void unsetMappedHomeSnssai();

    friend  void to_json(nlohmann::json& j, const AllowedSnssai& o);
    friend  void from_json(const nlohmann::json& j, AllowedSnssai& o);
protected:
    oai::model::Snssai m_AllowedSnssai;

    std::vector<oai::model::NsiInformation> m_NsiInformationList;
    bool m_NsiInformationListIsSet;
    oai::model::Snssai m_MappedHomeSnssai;
    bool m_MappedHomeSnssaiIsSet;
    
};

} // namespace oai::model

#endif /* AllowedSnssai_H_ */
