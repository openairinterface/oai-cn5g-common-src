/**
* Namf_EventExposure
* AMF Event Exposure Service.   © 2023, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.3
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#include "LossOfConnectivityReason.h"
#include "Helpers.h"

#include <sstream>

namespace oai::_3gpp::model
{

LossOfConnectivityReason::LossOfConnectivityReason()
{
    
}

void LossOfConnectivityReason::validate() const
{
    std::stringstream msg;
    if (!validate(msg))
    {
        throw oai::_3gpp::model::helpers::ValidationException(msg.str());
    }
}

bool LossOfConnectivityReason::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool LossOfConnectivityReason::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "LossOfConnectivityReason" : pathPrefix;

    
    if (!m_value.validate(msg))
    {
        success = false;
    }
    return success;
}

bool LossOfConnectivityReason::operator==(const LossOfConnectivityReason& rhs) const
{
    return
    
    getValue() == rhs.getValue()
    ;
}

bool LossOfConnectivityReason::operator!=(const LossOfConnectivityReason& rhs) const
{
    return !(*this == rhs);
}

void to_json(nlohmann::json& j, const LossOfConnectivityReason& o)
{
    j = nlohmann::json();
    to_json(j, o.m_value);
}

void from_json(const nlohmann::json& j, LossOfConnectivityReason& o)
{
    from_json(j, o.m_value);
}

LossOfConnectivityReason_anyOf LossOfConnectivityReason::getValue() const
{
    return m_value;
}

void LossOfConnectivityReason::setValue(LossOfConnectivityReason_anyOf value)
{
    m_value = value;
}

LossOfConnectivityReason_anyOf::eLossOfConnectivityReason_anyOf LossOfConnectivityReason::getEnumValue() const
{
    return m_value.getValue();
}

void LossOfConnectivityReason::setEnumValue(LossOfConnectivityReason_anyOf::eLossOfConnectivityReason_anyOf  value)
{
    m_value.setValue(value);
}

} // namespace oai::_3gpp::model

