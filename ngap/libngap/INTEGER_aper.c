...
asn_dec_rval_t INTEGER_decode_aper(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  if (ct && ct->flags != APC_UNCONSTRAINED) {
    ...
    if (ct->range_bits >= 0) {
      if (ct->range_bits > 16) {
        int max_range_bytes =
            (ct->range_bits >> 3) + (((ct->range_bits % 8) > 0) ? 1 : 0);
        int length = 0, i;
        long value = 0;
        ...
        if ((specs && specs->field_unsigned)
          ...
          ASN__DECODE_FAILED;
        ASN_DEBUG("Got value %ld + low %lld", value,
        ...
        value += ct->lower_bound;
        if ((specs && specs->field_unsigned) ? asn_ulong2INTEGER(st, value)
...
asn_enc_rval_t INTEGER_encode_aper(const asn_TYPE_descriptor_t *td,
                                   const asn_per_constraints_t *constraints,
  ...
  if (ct) {
    ...
    if (specs && specs->field_unsigned) {
      ...
      ASN_DEBUG("Value %lu (%02x/%zu) lb %lld ub %lld %s", uval, st->buf[0],
                st->size, (long long int)ct->lower_bound,
                (long long int)ct->upper_bound, inext ? "ext" : "fix");
      value = uval;
    } else {
      if (asn_INTEGER2long(st, &value))
        ASN__ENCODE_FAILED;
      ...
      ASN_DEBUG("Value %lu (%02x/%zu) lb %lld ub %lld %s", value, st->buf[0],
                st->size, (long long int)ct->lower_bound,
                (long long int)ct->upper_bound, inext ? "ext" : "fix");
    }
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    /* #10.5.6 */
    ASN_DEBUG("Encoding integer %ld (%lld) with range %d bits", value,
              (long long int)(value - ct->lower_bound), ct->range_bits);
