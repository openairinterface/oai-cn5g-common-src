...
static const void *element_ptr(const void *struct_ptr,
...
asn_dec_rval_t SEQUENCE_decode_oer(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  case 0: {
    ...
    ASN_DEBUG("Expecting preamble bits %" ASN_PRI_SIZE
  ...
  case 1: {
    ...
    microphase2_decode_continues:
      ...
      case RC_WMORE:
        ...
        RETURN(RC_WMORE);
      case RC_FAIL:
        ASN_DEBUG("Decoding failed at element %s \"%s\"", td->name, elm->name);
        RETURN(RC_FAIL);
      }
  ...
  case 2:
    ...
    NEXT_PHASE(ctx);
    ctx->step = (specs->first_extension < 0 ? td->elements_count
                                            : (size_t)specs->first_extension);
    /* Fall through */
  case 3:
 ...
 */
asn_enc_rval_t SEQUENCE_encode_oer(const asn_TYPE_descriptor_t *td,
                                   const asn_oer_constraints_t *constraints,
                                   const void *sptr,
