...
PduSessionReleaseReject::PduSessionReleaseReject()
    : Nas5gsmMessage(k5gsSessionManagementMessages, kPduSessionReleaseReject) {
...
int PduSessionReleaseReject::Encode(uint8_t* buf, int len) {
  oai::logger::logger_common::nas().debug(
      "Encoding PduSessionReleaseReject message");
  int encoded_size    = 0;
  ...
  oai::logger::logger_common::nas().debug(
      "Encoded PduSessionReleaseReject message len (%d)", encoded_size);
  return encoded_size;
}
...
int PduSessionReleaseReject::Decode(uint8_t* buf, int len) {
  oai::logger::logger_common::nas().debug(
      "Decoding PduSessionReleaseReject message");
  int decoded_size    = 0;
  ...
  oai::logger::logger_common::nas().debug(
      "Decoded PduSessionReleaseReject message len (%d)", decoded_size);
  return decoded_size;
}
