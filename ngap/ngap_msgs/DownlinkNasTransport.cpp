...
bool DownLinkNasTransportMsg::getRanPagingPriority(
  ...
  if (!m_RanPagingPriority.has_value())
    return false;
...
bool DownLinkNasTransportMsg::getMobilityRestrictionList(
  ...
  if (!m_MobilityRestrictionList.has_value())
    return false;
  ...
  for (int i = 0; i < m_DownLinkNasTransportIes->protocolIEs.list.count; i++) {
    ...
    case Ngap_ProtocolIE_ID_id_AMF_UE_NGAP_ID: {
      if (m_DownLinkNasTransportIes->protocolIEs.list.array[i]->criticality ==
          ...
          m_DownLinkNasTransportIes->protocolIEs.list.array[i]->value.present ==
    ...
    default: {
      oai::logger::logger_common::ngap().error("Decode NGAP message PDU error");
      return false;
    }
    }
  }
