...
int PduSessionAuthenticationResult::Encode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Encoding PduSessionAuthenticationResult message");
  int encoded_size    = 0;
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Encoded PduSessionAuthenticationResult message len (%d)",
          encoded_size);
  return encoded_size;
}
...
int PduSessionAuthenticationResult::Decode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Decoding PduSessionAuthenticationResult message");
  int decoded_size    = 0;
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Decoded PduSessionAuthenticationResult message len (%d)",
          decoded_size);
  return decoded_size;
}
