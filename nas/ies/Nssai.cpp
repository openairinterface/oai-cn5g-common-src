...
Nssai::Nssai(uint8_t iei, const std::vector<struct SNSSAI_s> &nssai)
  ...
  SetLengthIndicator((length > kNssaiContentMinimumLength)
                         ? length
...
int Nssai::Encode(uint8_t *buf, int len) const {
  ...
  for (int i = 0; i < s_nssais_.size(); i++) {
    ...
    if (s_nssais_.at(i).sd != SD_NO_VALUE) {
      ...
      oai::logger::logger_common::nas().debug("Encoded Nssai SD (0x%x)",
                                              s_nssais_.at(i).sd);
    }
    ...
    ENCODE_U8(buf + encoded_size - len_s_nssai - 1, len_s_nssai,
              encoded_size_tmp);
  }

  oai::logger::logger_common::nas().debug("Encoded Nssai len (%d)",
                                          encoded_size);
  return encoded_size;
}
...
int Nssai::Decode(const uint8_t *const buf, int len, bool is_iei) {
  ...
  for (int i = 0; i < s_nssais_.size(); i++) {
    oai::logger::logger_common::nas().debug("Decoded Nssai %s",
                                            s_nssais_.at(i).ToString().c_str());
  }
