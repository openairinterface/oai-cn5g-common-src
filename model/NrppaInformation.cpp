...
bool NrppaInformation::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "NrppaInformation" : pathPrefix;

            
    if (!m_NrppaPdu.validate(msg, _pathPrefix + ".nrppaPdu")) {
        msg << _pathPrefix << ": NrppaPdu is invalid;";
        success = false;
    }    
    return success;
}
...
oai::model::N2InfoContent NrppaInformation::getNrppaPdu() const
{
    return m_NrppaPdu;
}
void NrppaInformation::setNrppaPdu(oai::model::N2InfoContent const& value)
{
    m_NrppaPdu = value;
}
