...
int AuthenticationRequest::Encode(uint8_t *buf, int len) {
  ...
  if ((encoded_ie_size = NasHelper::Encode(ie_authentication_parameter_rand_,
  ...
  if ((encoded_ie_size = NasHelper::Encode(ie_authentication_parameter_autn_,
                                           buf, len, encoded_size)) ==
      KEncodeDecodeError) {
    return KEncodeDecodeError;
  }
...
int AuthenticationRequest::Decode(uint8_t *buf, int len) {
  ...
  while ((octet != 0x0)) {
    ...
    case kIeiAuthenticationParameterRand: {
      if ((decoded_ie_size =
               NasHelper::Decode(ie_authentication_parameter_rand_, buf, len,
    ...
    case kIeiAuthenticationParameterAutn: {
      if ((decoded_ie_size =
               NasHelper::Decode(ie_authentication_parameter_autn_, buf, len,
                                 decoded_size, true)) == KEncodeDecodeError) {
        return KEncodeDecodeError;
      }
      DECODE_U8_VALUE(buf, octet, decoded_size, len);
