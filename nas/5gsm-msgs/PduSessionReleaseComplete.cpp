...
PduSessionReleaseComplete::PduSessionReleaseComplete()
    : ie_header_(k5gsSessionManagementMessages, kPduSessionReleaseComplete) {
...
//------------------------------------------------------------------------------
uint32_t PduSessionReleaseComplete::GetLength() const {
  uint32_t msg_len = 0;
...
int PduSessionReleaseComplete::Encode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Encoding PduSessionReleaseComplete message");
  int encoded_size    = 0;
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Encoded PduSessionReleaseComplete message len (%d)", encoded_size);
  return encoded_size;
}
...
int PduSessionReleaseComplete::Decode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Decoding PduSessionReleaseComplete message");
  int decoded_size    = 0;
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Decoded PduSessionReleaseComplete message len (%d)", decoded_size);
  return decoded_size;
}
