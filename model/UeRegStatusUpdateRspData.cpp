/**
* Namf_Communication
* AMF Communication Service.   © 2023, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.4
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#include "UeRegStatusUpdateRspData.h"
#include "Helpers.h"

#include <sstream>

namespace oai::model
{

UeRegStatusUpdateRspData::UeRegStatusUpdateRspData()
{
    m_RegStatusTransferComplete = false;
    
}

void UeRegStatusUpdateRspData::validate() const
{
    std::stringstream msg;
    if (!validate(msg))
    {
        throw org::openapitools::server::helpers::ValidationException(msg.str());
    }
}

bool UeRegStatusUpdateRspData::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool UeRegStatusUpdateRspData::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "UeRegStatusUpdateRspData" : pathPrefix;

        
    return success;
}

bool UeRegStatusUpdateRspData::operator==(const UeRegStatusUpdateRspData& rhs) const
{
    return
    
    
    (isRegStatusTransferComplete() == rhs.isRegStatusTransferComplete())
    
    
    ;
}

bool UeRegStatusUpdateRspData::operator!=(const UeRegStatusUpdateRspData& rhs) const
{
    return !(*this == rhs);
}

void to_json(nlohmann::json& j, const UeRegStatusUpdateRspData& o)
{
    j = nlohmann::json::object();
    j["regStatusTransferComplete"] = o.m_RegStatusTransferComplete;
    
}

void from_json(const nlohmann::json& j, UeRegStatusUpdateRspData& o)
{
    j.at("regStatusTransferComplete").get_to(o.m_RegStatusTransferComplete);
    
}

bool UeRegStatusUpdateRspData::isRegStatusTransferComplete() const
{
    return m_RegStatusTransferComplete;
}
void UeRegStatusUpdateRspData::setRegStatusTransferComplete(bool const value)
{
    m_RegStatusTransferComplete = value;
}


} // namespace oai::model

