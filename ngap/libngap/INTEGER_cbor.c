 ...
 */
asn_enc_rval_t INTEGER_encode_cbor(
 ...
 */
asn_dec_rval_t INTEGER_decode_cbor(
  ...
  if (major == CBOR_MAJOR_UINT) {
    ...
    bstr_hlen = cbor_decode_head(
        buf + hlen, size - (size_t) hlen, &tag_major, &bstr_len);
    if (bstr_hlen < 0 || tag_major != CBOR_MAJOR_BYTES) ASN__DECODE_FAILED;
