...
int PduSessionModificationCommand::Encode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Encoding PduSessionModificationCommand message");
  int encoded_size    = 0;
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Encoded PduSessionModificationCommand message len (%d)",
          encoded_size);
  return encoded_size;
}
...
int PduSessionModificationCommand::Decode(uint8_t* buf, int len) {
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug("Decoding PduSessionModificationCommand message");
  int decoded_size    = 0;
  ...
  while ((octet != 0x0)) {
    ...
    switch (octet) {
      ...
      case kIeiSessionAmbr: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiSessionAmbr);
        if ((decoded_ie_size = NasHelper::Decode(
      ...
      case kIeiAuthorizedQosRules: {
        oai::logger::logger_registry::get_logger(LOGGER_COMMON)
            .debug("Decoding IEI 0x%x", kIeiAuthorizedQosRules);
        if ((decoded_ie_size = NasHelper::Decode(
  ...
  oai::logger::logger_registry::get_logger(LOGGER_COMMON)
      .debug(
          "Decoded PduSessionModificationCommand message len (%d)",
          decoded_size);
  return decoded_size;
}
