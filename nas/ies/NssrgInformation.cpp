...
//------------------------------------------------------------------------------
NssrgInformation::NssrgInformation() : Type6NasIe() {
  SetLengthIndicator(kNssrgInformationMinimumContentLength);
}

//------------------------------------------------------------------------------
NssrgInformation::NssrgInformation(uint8_t iei) : Type6NasIe(iei) {
  SetLengthIndicator(kNssrgInformationMinimumContentLength);
}

//------------------------------------------------------------------------------
void NssrgInformation::SetValue(const std::vector<uint8_t>& data) {
  value_               = data;
  uint16_t content_len = (static_cast<uint16_t>(data.size()) >
                          kNssrgInformationMinimumContentLength) ?
...
//------------------------------------------------------------------------------
const std::vector<uint8_t>& NssrgInformation::GetValue() const {
  return value_;
}

//------------------------------------------------------------------------------
int NssrgInformation::Encode(uint8_t* buf, int len) const {
...
//------------------------------------------------------------------------------
int NssrgInformation::Decode(const uint8_t* const buf, int len, bool is_iei) {
  ...
  if (len < (decoded_size + content_len)) {
