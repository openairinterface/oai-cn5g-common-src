...
//------------------------------------------------------------------------------
uint16_t QosRule::GetLengthIndicator() const { return length_; }
...
uint16_t QosRule::GetIeLength() const {
  return (GetLengthIndicator() +
          3); // 1 for QoS rule ID, 2 for length of QoSRule
}

//------------------------------------------------------------------------------
void QosRule::SetQosRuleId(uint8_t rule_id) { qos_rule_id_ = rule_id; }

//------------------------------------------------------------------------------
void QosRule::GetQosRuleId(uint8_t &rule_id) const { rule_id = qos_rule_id_; }

//------------------------------------------------------------------------------
uint8_t QosRule::GetQosRuleId() const { return qos_rule_id_; }
...
//------------------------------------------------------------------------------
uint8_t QosRule::GetRuleOperationCode() const { return rule_operation_code_; }

//------------------------------------------------------------------------------
void QosRule::SetDqrBit(bool dqr) { dqr_bit_ = dqr; }
//------------------------------------------------------------------------------
void QosRule::GetDqrBit(bool &dqr) const { dqr = dqr_bit_; }

//------------------------------------------------------------------------------
bool QosRule::GetDqrBit() const { return dqr_bit_; }
...
//------------------------------------------------------------------------------
std::optional<uint8_t> QosRule::GetPrecedence() const { return precedence_; }
...
//------------------------------------------------------------------------------
bool QosRule::GetSegregation() const { return segregation_; }
...
//------------------------------------------------------------------------------
void QosRule::GetQfi(std::optional<uint8_t> &qfi) const { qfi = qfi_; }

//------------------------------------------------------------------------------
std::optional<uint8_t> QosRule::GetQfi() const { return qfi_; }
...
int QosRule::Encode(uint8_t *buf, int len) const {
    ...
    if (rule_operation_code_ ==
      ...
      if (pf_modify_and_delete_list_.has_value()) {
        for (auto p : pf_modify_and_delete_list_.value()) {
          ENCODE_U8(buf + encoded_size, (p.packet_filter_id & 0x0f),
                    encoded_size);
        }
      }
      ...
      if (pf_create_and_modify_and_replace_list_.has_value()) {
        ...
        ENCODE_U8(buf + encoded_size, p.content.length,
        ...
        for (auto const &pc : p.content.packet_filter_components) {
          ...
          int encoded_content_size =
              encode_bstring(pc.value, (buf + encoded_size),
  ...
  if (segregation_)
    octet_segregation_qfi = 0b01000000;
  if (qfi_.has_value())
  ...
  oai::logger::logger_common::nas().debug("Encoded QosRule, len (%d)",
                                          encoded_size);
  return encoded_size;
}
...
int QosRule::Decode(const uint8_t *const buf, int len) {
  ...
  if ((rule_operation_code_ == kQosRuleRuleOperationCodeCreateNewQosRule) or
    ...
    for (int i = 0; i < number_of_packet_filters_; i++) {
      ...
      DECODE_U8(buf + decoded_size, pf.content.length,
      ...
      while (packet_filter_length > 0) {
        ...
        case kQosRulePfctiIpv4LocalAddressType: {
          decoded_ie_size = decode_bstring(&pc.value, 8, (buf + decoded_size),
                                           len - decoded_size);
        ...
        case kQosRulePfctiIpv6LocalAddressOrPrefixLengthType: {
          decoded_ie_size = decode_bstring(&pc.value, 17, (buf + decoded_size),
                                           len - decoded_size);
        ...
        case kQosRulePfctiProtocolIdentifierOrNextHeaderType: {
          decoded_ie_size = decode_bstring(&pc.value, 1, (buf + decoded_size),
                                           len - decoded_size);
        ...
        case kQosRulePfctiSingleRemotePortType: {
          ...
          decoded_ie_size = decode_bstring(&pc.value, 2, (buf + decoded_size),
                                           len - decoded_size);
        } break;
        ...
        case kQosRulePfctiRemotePortRangeType: {
          ...
          decoded_ie_size = decode_bstring(&pc.value, 4, (buf + decoded_size),
                                           len - decoded_size);
        } break;
        ...
        case kQosRulePfctiFlowLabelType: {
          ...
          decoded_ie_size = decode_bstring(&pc.value, 3, (buf + decoded_size),
                                           len - decoded_size);
        } break;
        ...
        case kQosRulePfctiSourceMacAddressType: {
          ...
          decoded_ie_size = decode_bstring(&pc.value, 6, (buf + decoded_size),
                                           len - decoded_size);
        } break;
  ...
  oai::logger::logger_common::nas().debug("Decoded QosRule (len %d)",
                                          decoded_size);
  return decoded_size;
}
