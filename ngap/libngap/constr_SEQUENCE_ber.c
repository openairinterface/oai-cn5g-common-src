 ...
 */
asn_dec_rval_t SEQUENCE_decode_ber(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  case 1:
    ...
    microphase2:
      ...
      if (elements[edx].flags & ATF_OPEN_TYPE) {
        rval =
            OPEN_TYPE_ber_get(opt_codec_ctx, td, st, &elements[edx], ptr, LEFT);
      } else {
        rval = elements[edx].type->op->ber_decoder(
 ...
 */
asn_enc_rval_t SEQUENCE_encode_der(const asn_TYPE_descriptor_t *td,
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    ASN_DEBUG("Member %" ASN_PRI_SIZE " %s estimated %ld bytes", edx, elm->name,
              (long)erval.encoded);
  }
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    tmperval = elm->type->op->der_encoder(elm->type, *memb_ptr2, elm->tag_mode,
                                          elm->tag, cb, app_key);
