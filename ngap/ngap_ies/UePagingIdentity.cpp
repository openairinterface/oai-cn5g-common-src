...
//------------------------------------------------------------------------------
void UePagingIdentity::set(const std::string &set_id,
...
//------------------------------------------------------------------------------
void UePagingIdentity::get(std::string &set_id, std::string &pointer,
...
bool UePagingIdentity::encode(Ngap_UEPagingIdentity_t &pdu) const {
  ...
  if (!m_FiveGSTmsi.encode(*pdu.choice.fiveG_S_TMSI))
    return false;
...
bool UePagingIdentity::decode(const Ngap_UEPagingIdentity_t &pdu) {
  if (pdu.present != Ngap_UEPagingIdentity_PR_fiveG_S_TMSI)
    return false;
  if (!m_FiveGSTmsi.decode(*pdu.choice.fiveG_S_TMSI))
    return false;
