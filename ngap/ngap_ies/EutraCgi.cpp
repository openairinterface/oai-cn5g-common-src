...
//------------------------------------------------------------------------------
void EutraCgi::set(const PlmnId &plmnId,
                   const EutraCellIdentity &eutraCellIdentity) {
  m_PlmnId = plmnId;
...
bool EutraCgi::encode(Ngap_EUTRA_CGI_t &eutraCgi) const {
  if (!m_PlmnId.encode(eutraCgi.pLMNIdentity))
    return false;
  if (!m_eutraCellIdentity.encode(eutraCgi.eUTRACellIdentity))
    return false;
...
bool EutraCgi::decode(const Ngap_EUTRA_CGI_t &eutraCgi) {
  if (!m_PlmnId.decode(eutraCgi.pLMNIdentity))
    return false;
  if (!m_eutraCellIdentity.decode(eutraCgi.eUTRACellIdentity))
    return false;
  return true;
}
