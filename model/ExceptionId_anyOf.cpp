...
bool ExceptionId_anyOf::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "ExceptionId_anyOf" : pathPrefix;

    
    if (m_value == ExceptionId_anyOf::eExceptionId_anyOf::INVALID_VALUE_OPENAPI_GENERATED)
    {
        success = false;
...
void from_json(const nlohmann::json& j, ExceptionId_anyOf& o)
{
    ...
    if (s == "UNEXPECTED_UE_LOCATION") {
     ...
     o.setValue(ExceptionId_anyOf::eExceptionId_anyOf::UNEXPECTED_LONG_LIVE_FLOW);
    } 
    else if (s == "UNEXPECTED_LARGE_RATE_FLOW") {
     o.setValue(ExceptionId_anyOf::eExceptionId_anyOf::UNEXPECTED_LARGE_RATE_FLOW);
    } 
     ...
     o.setValue(ExceptionId_anyOf::eExceptionId_anyOf::WRONG_DESTINATION_ADDRESS);
    } 
    else if (s == "TOO_FREQUENT_SERVICE_ACCESS") {
     o.setValue(ExceptionId_anyOf::eExceptionId_anyOf::TOO_FREQUENT_SERVICE_ACCESS);
    } 
    else if (s == "UNEXPECTED_RADIO_LINK_FAILURES") {
     o.setValue(ExceptionId_anyOf::eExceptionId_anyOf::UNEXPECTED_RADIO_LINK_FAILURES);
    } 
