 ...
 */
ssize_t CC_PRINTFLIKE(3, 4) asn__format_to_callback(
    int (*callback)(const void*, size_t, void* key), void* key, const char* fmt,
    ...);
...
#define ASN__CALLBACK2(buf1, size1, buf2, size2)                               \
  ASN__E_CALLBACK(                                                             \
      (size1) + (size2), ASN__E_cbc(buf1, size1) || ASN__E_cbc(buf2, size2))
#define ASN__CALLBACK3(buf1, size1, buf2, size2, buf3, size3)                  \
  ASN__E_CALLBACK(                                                             \
      (size1) + (size2) + (size3), ASN__E_cbc(buf1, size1) ||                  \
 ...
 * When enabled, the XER decoder treats empty tags (e.g., <field></field> or
 * <field/>) in OPTIONAL fields as absent rather than invalid. Define
 * XER_ALLOW_EMPTY_OPTIONALS at compile time to enable this feature:
 ...
 */
static inline int asn_is_meta_syntax_keyword(const char* name) {
  if (!name) return 0;
