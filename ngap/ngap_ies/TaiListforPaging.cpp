...
//------------------------------------------------------------------------------
void TaiListForPaging::set(const std::vector<Tai> &list) { m_TaiList = list; }

//------------------------------------------------------------------------------
void TaiListForPaging::get(std::vector<Tai> &list) const { list = m_TaiList; }
//------------------------------------------------------------------------------
bool TaiListForPaging::encode(Ngap_TAIListForPaging_t &pdu) const {
  for (auto &tai : m_TaiList) {
    ...
    if (!tai.encode(ta->tAI))
      return false;
    if (ASN_SEQUENCE_ADD(&pdu.list, ta) != 0)
      return false;
  }
  return true;
}
...
bool TaiListForPaging::decode(const Ngap_TAIListForPaging_t &pdu) {
  ...
  for (int i = 0; i < pdu.list.count; i++) {
    Tai tai = {};
    if (!tai.decode(pdu.list.array[i]->tAI))
      return false;
