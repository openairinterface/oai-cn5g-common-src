...
asn_dec_rval_t OPEN_TYPE_cbor_get(
  ...
  if (elm->type->elements_count > 0) {
    ...
    if (selected.presence_index > elm->type->elements_count) {
      ASN_DEBUG(
          "Open Type %s->%s: presence index %u out of bounds", td->name,
          elm->name, selected.presence_index);
      ASN__DECODE_FAILED;
    }

    if (!elm->type->elements) {
      ASN_DEBUG(
          "Open Type %s->%s: elements array is NULL", td->name, elm->name);
      ASN__DECODE_FAILED;
    }
  ...
  if (!selected.type_descriptor->op->cbor_decoder) {
    ASN_DEBUG(
        "Open Type %s->%s: selected type %s has no CBOR decoder", td->name,
...
asn_enc_rval_t OPEN_TYPE_cbor_put(
  ...
  if (!selected.presence_index) {
    ASN_DEBUG(
        "Open Type %s->%s: type_selector returned presence_index=0", td->name,
        elm->name);
    ASN__ENCODE_FAILED;
  }
  ...
  if (!selected.type_descriptor->op ||
      !selected.type_descriptor->op->cbor_encoder) {
    ASN_DEBUG(
        "Open Type %s->%s: selected type %s has no CBOR encoder", td->name,
        elm->name, selected.type_descriptor->name);
    ASN__ENCODE_FAILED;
  }
  ...
  if (elm->type->elements_count > 0) {
    ...
    if (variant_elm->flags & ATF_POINTER) {
      variant_memb_ptr = *(
          const void* const*) ((const char*) memb_ptr + variant_elm->memb_offset);
      if (!variant_memb_ptr) {
        ASN_DEBUG(
            "Open Type %s->%s: variant data pointer is NULL", td->name,
            elm->name);
        ASN__ENCODE_FAILED;
      }
    } else {
