/*-
 * Runtime helpers for asn_cval_t constraint bounds.  See
 * asn_constraint_value.h.
 */
...
static int acv_bytes_negative(const uint8_t* buf, size_t size) {
 ...
 */
static void acv_bytes_canonical(
    const uint8_t* buf, size_t size, const uint8_t** out_buf,
    size_t* out_size) {
  if (size == 0) {
    *out_buf  = buf;
    *out_size = 0;
...
static int acv_cmp_bytes(
    const uint8_t* ab, size_t as, const uint8_t* bb, size_t bs) {
  int an, bn;
...
static int acv_sint_to_INTEGER(intmax_t v, INTEGER_t* tmp) {
  memset(tmp, 0, sizeof(*tmp));
  return asn_imax2INTEGER(tmp, v);
}
static int acv_uint_to_INTEGER(uintmax_t v, INTEGER_t* tmp) {
  memset(tmp, 0, sizeof(*tmp));
  return asn_umax2INTEGER(tmp, v);
}

/* Compare a materialized scalar (as INTEGER_t bytes) against cval bytes. */
static int acv_cmp_scalarbytes_vs_cvalbytes(
    const INTEGER_t* s, const asn_cval_t* b) {
...
static int acv_to_INTEGER(
    const asn_cval_t* v, INTEGER_t* tmp, int* materialized) {
  *materialized = 0;
  switch (v->kind) {
    case ACV_SINT:
...
int asn_cval_cmp(const asn_cval_t* a, const asn_cval_t* b) {
  ...
  if (a->kind == ACV_INTEGER_BYTES) {
    ...
    if (b->kind == ACV_INTEGER_BYTES)
      return acv_cmp_bytes(
          a->value.b.buf, a->value.b.size, b->value.b.buf, b->value.b.size);
...
int asn_INTEGER_cmp_cval(
  ...
  if (a_is_unsigned) {
    uintmax_t uv;
    if (asn_INTEGER2umax(a, &uv) == 0) return asn_cval_cmp_uint(uv, b);
  } else {
    intmax_t sv;
    if (asn_INTEGER2imax(a, &sv) == 0) return asn_cval_cmp_sint(sv, b);
  }
  /* Value does not fit a native scalar: compare canonical octets. */
  {
    ...
    int mat = 0, r;
    if (b->kind == ACV_INTEGER_BYTES) {
      return acv_cmp_bytes(a->buf, a->size, b->value.b.buf, b->value.b.size);
    }
    if (acv_to_INTEGER(b, &tb, &mat) != 0) return 0;
...
static int acv_as_imax(const asn_cval_t* v, intmax_t* out) {
  switch (v->kind) {
    case ACV_SINT:
      *out = v->value.s;
      return 0;
