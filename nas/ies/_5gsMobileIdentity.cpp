...
int _5gsMobileIdentity::Encode(uint8_t *buf, int len) const {
  ...
  default: {
    oai::logger::logger_common::nas().debug("Unknown Type of Identity  0x%x",
                                            type_of_identity_);
  }
  }
...
int _5gsMobileIdentity::Decode(const uint8_t *const buf, int len, bool is_iei) {
  ...
  // Type of Identity
  DECODE_U8(buf + decoded_size, octet,
            decoded_size_tmp); // Decode but don't increase decoded_size

  switch (octet & 0x07) {
  case kSuci: {
    type_of_identity_ = kSuci;
    decoded_size += DecodeSuci(buf + decoded_size, len - decoded_size, ie_len);
    oai::logger::logger_common::nas().debug("Decoded SUCI (%d octets)",
                                            decoded_size);
  } break;
  case k5gGuti: {
  ...
  oai::logger::logger_common::nas().debug("Decoded %s, len (%ld)",
...
int _5gsMobileIdentity::Encode5gGuti(uint8_t *buf, int len) const {
  oai::logger::logger_common::nas().debug("Encoding 5G-GUTI IEI 0x%x",
                                          iei_.value());
  int encoded_size = 0;
  ...
  // Type of Identity
  ENCODE_U8(buf + encoded_size, 0xf0 | k5gGuti,
  ...
  ENCODE_U8(buf + encoded_size, ((_5g_guti_.value().amf_set_id & 0x03ff) >> 2),
            encoded_size);
  ENCODE_U8(buf + encoded_size,
  ...
  oai::logger::logger_common::nas().debug("Encoded 5G-GUTI IE (len %ld octets)",
                                          encoded_size);
  return encoded_size;
}
...
int _5gsMobileIdentity::Decode5gGuti(const uint8_t *const buf, int len) {
  ...
  oai::logger::logger_common::nas().debug("MCC %s, MNC %s", tmp.mcc.c_str(),
                                          tmp.mnc.c_str());
...
//------------------------------------------------------------------------------
void _5gsMobileIdentity::Set5gGuti(const std::string &mcc,
                                   const std::string &mnc,
                                   uint8_t amf_region_id, uint16_t amf_set_id,
                                   uint8_t amf_pointer, uint32_t _5g_tmsi) {
...
int _5gsMobileIdentity::EncodeSuci(uint8_t *buf, int len) const {
  ...
  if (!supi_format_imsi_.has_value())
    return KEncodeDecodeError;
  ...
  ENCODE_U8(buf + encoded_size,
            (0x70 & (kSupiFormatImsi << 4)) | (0x07 & kSuci), encoded_size);
  ...
  // Routing Indicator
  encoded_size +=
      EncodeRoutingIndicator(supi_format_imsi_.value().routing_indicator,
                             buf + encoded_size, len - encoded_size);

  // Protection Scheme
  ENCODE_U8(buf + encoded_size,
            0x0f & supi_format_imsi_.value().protection_scheme_id,
            encoded_size);

  // Home Network Public Key Identifier
  ENCODE_U8(buf + encoded_size, supi_format_imsi_.value().home_network_pki,
            encoded_size);
  ...
  for (int i = 0; i < supi_format_imsi_.value().msin.length() / 2; i++) {
    conv::string_to_int8(supi_format_imsi_.value().msin.substr(i, 1),
                         digit_low);
    conv::string_to_int8(supi_format_imsi_.value().msin.substr(i + 1, 1),
                         digit_high);
    uint8_t octet = (0xf0 & (digit_high << 4)) | (digit_low & 0x0f);
    ENCODE_U8(buf + encoded_size, octet, encoded_size);
  }
  if (supi_format_imsi_.value().msin.length() % 2 != 0) {
    conv::string_to_int8(supi_format_imsi_.value().msin.substr(
  ...
  oai::logger::logger_common::nas().debug("Encoded SUCI IE (len %d octets)",
                                          encoded_size);
  return encoded_size;
}

//------------------------------------------------------------------------------
int _5gsMobileIdentity::DecodeSuci(const uint8_t *const buf, int len,
                                   int ie_len) {
  ...
  case kSupiFormatImsi: {
    ...
    decoded_size += nas_utils::decodeMccMncFromBuffer(
        supi_format_imsi_tmp.mcc, supi_format_imsi_tmp.mnc, buf + decoded_size,
        len - decoded_size);
    oai::logger::logger_common::nas().debug("MCC %s, MNC %s",
                                            supi_format_imsi_tmp.mcc.c_str(),
    ...
    oai::logger::logger_common::nas().debug("Decoded MSIN %s",
                                            supi_format_imsi_tmp.msin.c_str());
...
//------------------------------------------------------------------------------
void _5gsMobileIdentity::SetSuciWithSupiImsi(const std::string &mcc,
                                             const std::string &mnc,
...
int _5gsMobileIdentity::EncodeRoutingIndicator(
  ...
  oai::logger::logger_common::nas().debug("Routing Indicator (%s)",
                                          routing_indicator.value().c_str());
...
//------------------------------------------------------------------------------
int _5gsMobileIdentity::EncodeMsin(const std::string &msin_str, uint8_t *buf,
                                   int len) const {
  oai::logger::logger_common::nas().warn(
...
int _5gsMobileIdentity::Encode5gSTmsi(uint8_t *buf, int len) const {
  ...
  ENCODE_U8(buf + encoded_size,
            ((_5g_s_tmsi_.value().amf_set_id) & 0x03fc) >> 2, encoded_size);
  ENCODE_U8(buf + encoded_size,
...
//------------------------------------------------------------------------------
void _5gsMobileIdentity::Set5gSTmsi(uint16_t amf_set_id, uint8_t amf_pointer,
...
//------------------------------------------------------------------------------
bool _5gsMobileIdentity::Get5gSTmsi(uint16_t &amf_set_id, uint8_t &amf_pointer,
...
int _5gsMobileIdentity::EncodeImeisv(uint8_t *buf, int len) const {
  ...
  while (i < imeisv_.value().identity.length()) {
    if (i == 0) {
      ...
      conv::string_to_int8(imeisv_.value().identity.substr(i + 1, 1),
                           digit_high);
      i++;
    } else if (i == imeisv_.value().identity.length() - 1) {
      conv::string_to_int8(imeisv_.value().identity.substr(i, 1), digit_low);
  ...
  oai::logger::logger_common::nas().debug("Encoded IMEISV IE (len %d octets)",
                                          encoded_size);
  return encoded_size;
}
