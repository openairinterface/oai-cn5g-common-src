...
bool UeInitiatedResourceRequest::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "UeInitiatedResourceRequest" : pathPrefix;

            
    if (!m_RuleOp.validate(msg, _pathPrefix + ".ruleOp")) {
        msg << _pathPrefix << ": RuleOp is invalid;";
        success = false;
    }         
    
    /* PackFiltInfo */ {
        const std::vector<oai::model::PacketFilterInfo>& value = m_PackFiltInfo;
        ...
        { // Recursive validation of array elements
            ...
            int i = 0;
            for (const oai::model::PacketFilterInfo& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".packFiltInfo") && success;
...
bool UeInitiatedResourceRequest::operator==(const UeInitiatedResourceRequest& rhs) const
{
    return
    ...
    ((!reqQosIsSet() && !rhs.reqQosIsSet()) || (reqQosIsSet() && rhs.reqQosIsSet() && getReqQos() == rhs.getReqQos()))
...
oai::model::RuleOperation UeInitiatedResourceRequest::getRuleOp() const
{
    return m_RuleOp;
}
void UeInitiatedResourceRequest::setRuleOp(oai::model::RuleOperation const& value)
{
    m_RuleOp = value;
}
...
std::vector<oai::model::PacketFilterInfo> UeInitiatedResourceRequest::getPackFiltInfo() const
{
    return m_PackFiltInfo;
}
void UeInitiatedResourceRequest::setPackFiltInfo(std::vector<oai::model::PacketFilterInfo> const& value)
{
    m_PackFiltInfo = value;
}
oai::model::RequestedQos UeInitiatedResourceRequest::getReqQos() const
{
    return m_ReqQos;
}
void UeInitiatedResourceRequest::setReqQos(oai::model::RequestedQos const& value)
{
    m_ReqQos = value;
    m_ReqQosIsSet = true;
}
