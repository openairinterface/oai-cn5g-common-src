...
asn_dec_rval_t OCTET_STRING_decode_aper(
  ...
  if (csiz->effective_bits == 0) {
    ...
    if (st->size > 2 || csiz->range_bits != 0) {
      if (aper_get_align(pd) < 0) RETURN(RC_FAIL);
    }
    if (bpc) {
      ASN_DEBUG(
          "Decoding OCTET STRING size %lld", (long long int) csiz->upper_bound);
      ...
      ASN_DEBUG(
          "Decoding BIT STRING size %lld", (long long int) csiz->upper_bound);
  ...
  do {
    ...
    ASN_DEBUG(
        ...
        (long) raw_len, repeat ? "repeat" : "once", td->name);
    ...
    if (bpc) {
      ret = OCTET_STRING_per_get_characters(
          pd, &st->buf[st->size], raw_len, bpc, unit_bits, cval->lower_bound,
          cval->upper_bound, pc);
      if (ret > 0) RETURN(RC_FAIL);
    } else {
...
asn_enc_rval_t OCTET_STRING_encode_aper(
  ...
  ASN_DEBUG(
      ...
      td->name, sizeinunits, unit_bits, (long long int) csiz->lower_bound,
      (long long int) csiz->upper_bound, csiz->effective_bits,
      ct_extensible ? " EXT" : "");
  ...
  if (csiz->effective_bits >= 0) {
    ASN_DEBUG(
        "Encoding %zu bytes (%lld), length in %d bits", st->size,
        (long long int) (sizeinunits - csiz->lower_bound),
        csiz->effective_bits);
    if (csiz->effective_bits > 0) {
      ret = aper_put_length(
          po, csiz->lower_bound, csiz->upper_bound,
    ...
    if (csiz->effective_bits > 0 || (st->size > 2) ||
        ...
        (csiz->range_bits !=
         0)) { /* X.691 #16 NOTE 1 for fixed length (<=16 bits) strings*/
      if (aper_put_align(po) < 0) ASN__ENCODE_FAILED;
    }
    if (bpc) {
      ret = OCTET_STRING_per_put_characters(
          po, st->buf, sizeinunits, bpc, unit_bits, cval->lower_bound,
          cval->upper_bound, pc);
    } else {
