/**
* Nsmf_PDUSession
* SMF PDU Session Service.   © 2025, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.7
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#include "RedundantPduSessionInformation.h"
#include "Helpers.h"

#include <sstream>

namespace oai::model
{

RedundantPduSessionInformation::RedundantPduSessionInformation()
{
    m_PduSessionPairId = 0;
    m_PduSessionPairIdIsSet = false;
    
}

void RedundantPduSessionInformation::validate() const
{
    std::stringstream msg;
    if (!validate(msg))
    {
        throw org::openapitools::server::helpers::ValidationException(msg.str());
    }
}

bool RedundantPduSessionInformation::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool RedundantPduSessionInformation::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "RedundantPduSessionInformation" : pathPrefix;

        
    if (!m_Rsn.validate(msg, _pathPrefix + ".rsn")) {
        msg << _pathPrefix << ": Rsn is invalid;";
        success = false;
    }     
    if (pduSessionPairIdIsSet())
    {
        const int32_t& value = m_PduSessionPairId;
        const std::string currentValuePath = _pathPrefix + ".pduSessionPairId";
                
        
        if (value < 0)
        {
            success = false;
            msg << currentValuePath << ": must be greater than or equal to 0;";
        }
        if (value > 255)
        {
            success = false;
            msg << currentValuePath << ": must be less than or equal to 255;";
        }

    }
    
    return success;
}

bool RedundantPduSessionInformation::operator==(const RedundantPduSessionInformation& rhs) const
{
    return
    
    
    (getRsn() == rhs.getRsn())
     &&
    
    
    ((!pduSessionPairIdIsSet() && !rhs.pduSessionPairIdIsSet()) || (pduSessionPairIdIsSet() && rhs.pduSessionPairIdIsSet() && getPduSessionPairId() == rhs.getPduSessionPairId()))
    
    ;
}

bool RedundantPduSessionInformation::operator!=(const RedundantPduSessionInformation& rhs) const
{
    return !(*this == rhs);
}

void to_json(nlohmann::json& j, const RedundantPduSessionInformation& o)
{
    j = nlohmann::json::object();
    j["rsn"] = o.m_Rsn;
    if(o.pduSessionPairIdIsSet())
        j["pduSessionPairId"] = o.m_PduSessionPairId;
    
}

void from_json(const nlohmann::json& j, RedundantPduSessionInformation& o)
{
    j.at("rsn").get_to(o.m_Rsn);
    if(j.find("pduSessionPairId") != j.end())
    {
        j.at("pduSessionPairId").get_to(o.m_PduSessionPairId);
        o.m_PduSessionPairIdIsSet = true;
    } 
    
}

oai::model::Rsn RedundantPduSessionInformation::getRsn() const
{
    return m_Rsn;
}
void RedundantPduSessionInformation::setRsn(oai::model::Rsn const& value)
{
    m_Rsn = value;
}
int32_t RedundantPduSessionInformation::getPduSessionPairId() const
{
    return m_PduSessionPairId;
}
void RedundantPduSessionInformation::setPduSessionPairId(int32_t const value)
{
    m_PduSessionPairId = value;
    m_PduSessionPairIdIsSet = true;
}
bool RedundantPduSessionInformation::pduSessionPairIdIsSet() const
{
    return m_PduSessionPairIdIsSet;
}
void RedundantPduSessionInformation::unsetPduSessionPairId()
{
    m_PduSessionPairIdIsSet = false;
}


} // namespace oai::model

