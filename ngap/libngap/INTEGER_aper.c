...
asn_dec_rval_t INTEGER_decode_aper(
  ...
  int repeat;
  int inext =
      0; /* Track if value is in extension part (outside constraint range) */
  ...
  if (ct && ct->flags & APC_EXTENSIBLE) {
    ...
    if (inext) {
       ...
       * preserve lower_bound while allowing unconstrained length
       * (range_bits=-1).
       *
    ...
    if (ct->range_bits >= 0) {
      if (ct->range_bits > 16) {
        ...
        while (len > 0) {
          ...
          if (offset > (UINTMAX_MAX >> 8)) ASN__DECODE_FAILED;
          offset = (offset << 8) | (unsigned) buf;
          len--;
        }
        ...
        if (ct->upper_bound < ct->lower_bound) {
          ...
          uintmax_t range;
          if (ct->lower_bound >= 0) {
            range = (uintmax_t) ct->upper_bound - (uintmax_t) ct->lower_bound;
          } else {
            uintmax_t neg_lb = (uintmax_t) (-(ct->lower_bound + 1)) + 1;
            if ((uintmax_t) ct->upper_bound > UINTMAX_MAX - neg_lb)
              ASN__DECODE_FAILED;
            range = (uintmax_t) ct->upper_bound + neg_lb;
          }
          if (offset > range) ASN__DECODE_FAILED;
        }

        if (ct->lower_bound >= 0) {
          if (offset > (uintmax_t) INTMAX_MAX - (uintmax_t) ct->lower_bound)
            ASN__DECODE_FAILED;
          value = (intmax_t) ((uintmax_t) ct->lower_bound + offset);
        } else {
          uintmax_t neg_lb = (uintmax_t) (-(ct->lower_bound + 1)) + 1;
          if (offset >= neg_lb) {
            uintmax_t positive = offset - neg_lb;
            if (positive > (uintmax_t) INTMAX_MAX) ASN__DECODE_FAILED;
            value = (intmax_t) positive;
          } else {
            uintmax_t negative = neg_lb - offset;
            if (negative > (uintmax_t) INTMAX_MAX + 1) ASN__DECODE_FAILED;
            if (negative == (uintmax_t) INTMAX_MAX + 1) {
              value = INTMAX_MIN;
            } else {
              value = -(intmax_t) negative;
            }
          }
        }

        if (specs && specs->field_unsigned && value < 0) {
          ASN__DECODE_FAILED;
        }
        ...
        if (specs && specs->field_unsigned) {
          if ((uintmax_t) value > (uintmax_t) ct->upper_bound)
            ASN__DECODE_FAILED;
          if (asn_umax2INTEGER(st, (uintmax_t) value)) ASN__DECODE_FAILED;
        } else {
          if (value < ct->lower_bound || value > ct->upper_bound)
            ASN__DECODE_FAILED;
          if (asn_imax2INTEGER(st, value)) ASN__DECODE_FAILED;
        }

        ASN_DEBUG(
            "Got value %" ASN_PRIdMAX " + low %" ASN_PRIdMAX "", value,
  ...
  if (ct && ct->lower_bound) {
     ...
     */
    long value;
    if (asn_INTEGER2long(st, &value)) ASN__DECODE_FAILED;
    if (asn_long2INTEGER(st, value + ct->lower_bound)) ASN__DECODE_FAILED;
  }

  return rval;
}

asn_enc_rval_t INTEGER_encode_aper(
  ...
  if (ct) {
    ...
    if (specs && specs->field_unsigned) {
      uintmax_t uval;
      if (asn_INTEGER2umax(st, &uval)) ASN__ENCODE_FAILED;
      /* Check proper range */
      if (ct->flags & APC_SEMI_CONSTRAINED) {
        if (uval < (uintmax_t) ct->lower_bound) inext = 1;
      ...
      ASN_DEBUG(
          "Value %" ASN_PRIdMAX " (%02x/%" ASN_PRI_SIZE ") lb %" ASN_PRIdMAX
          " ub %" ASN_PRIdMAX " %s",
          uval, st->buf[0], st->size, (intmax_t) ct->lower_bound,
          (intmax_t) ct->upper_bound, inext ? "ext" : "fix");
      value = uval;
    } else {
      ...
      if (ct->flags & APC_SEMI_CONSTRAINED) {
        ...
        if (value < ct->lower_bound || value > ct->upper_bound) inext = 1;
      }
      ASN_DEBUG(
          "Value %" ASN_PRIdMAX " (%02x/%" ASN_PRI_SIZE ") lb %" ASN_PRIdMAX
          " ub %" ASN_PRIdMAX " %s",
          value, st->buf[0], st->size, (intmax_t) ct->lower_bound,
          (intmax_t) ct->upper_bound, inext ? "ext" : "fix");
    }
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    /* #10.5.6 */
    ASN_DEBUG(
        "Encoding integer %" ASN_PRIdMAX " (%" ASN_PRIdMAX
        ") with range %d bits",
        value, (intmax_t) (value - ct->lower_bound), ct->range_bits);
    ...
    if (ct->range_bits < 8 || (ct->range_bits == 8 && range < 256)) {
      if (per_put_few_bits(po, 0x00 | v, ct->range_bits)) ASN__ENCODE_FAILED;
    } else if (ct->range_bits == 8) {
      ...
      if (aper_put_align(po) < 0) ASN__ENCODE_FAILED;
      if (per_put_few_bits(po, 0x0000 | v, 16)) ASN__ENCODE_FAILED;
    } else {
      /* X.691 13.2.6: constrained whole number with range > 65536. */
      size_t max_range_bytes = ((size_t) ct->range_bits + 7) >> 3;
      size_t num_bytes       = 0;
      ...
      if (max_range_bytes > sizeof(buf)) ASN__ENCODE_FAILED;
      ...
      if (per_put_few_bits(po, num_bytes - 1, length_bits)) ASN__ENCODE_FAILED;
      if (aper_put_align(po) < 0) ASN__ENCODE_FAILED;

      ASN_DEBUG(
          "Constrained INTEGER>16 encode: range_bits=%d "
          "max_bytes=%" ASN_PRI_SIZE " len=%" ASN_PRI_SIZE
          " offset=%" ASN_PRIuMAX,
      ...
      if (per_put_many_bits(po, buf, 8 * num_bytes)) ASN__ENCODE_FAILED;
    }
    ASN__ENCODED_OK(er);
  }
  ...
  if (ct && ct->lower_bound) {
    ASN_DEBUG(
        "Adjust lower bound to %" ASN_PRIdMAX "", (intmax_t) ct->lower_bound);
    /*
    ...
    if (specs && specs->field_unsigned) {
      ...
      if (uval < (uintmax_t) ct->lower_bound) ASN__ENCODE_FAILED;
      ...
      intmax_t sval;
      if (asn_INTEGER2imax(st, &sval)) ASN__ENCODE_FAILED;
      /* Check that value is >= lower_bound */
      if (sval < ct->lower_bound) ASN__ENCODE_FAILED;
      sval -= ct->lower_bound;
  ...
  for (buf = st->buf, end = st->buf + st->size; buf < end;) {
    ...
    if (per_put_many_bits(po, buf, 8 * mayEncode)) ASN__ENCODE_FAILED;
    buf += mayEncode;
    if (need_eom && (aper_put_length(po, -1, -1, 0, NULL) < 0))
      ASN__ENCODE_FAILED;
  }
