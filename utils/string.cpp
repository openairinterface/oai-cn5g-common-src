...
//
std::string oai::utils::get_query_param(std::string querystring,
                                        std::string param) {
  std::regex reList("([^=]*)=([^&]*)&?");
...
bool oai::utils::bstring_to_ipv4(bstring str, struct in_addr ipv4_address) {
  if ((str->slen != 4) or (str->data == nullptr))
    return false;
...
void oai::utils::ipv6_to_bstring(struct in6_addr ipv6_address, bstring str) {
  ...
  if (inet_ntop(AF_INET6, &ipv6_address, str_addr6, sizeof(str_addr6))) {
    ...
    oai::logger::logger_common::common().info(" Ipv6 address....: %s",
                                              ipv6_addr_str.c_str());
    unsigned char buf_in6_addr[sizeof(struct in6_addr)];
    if (inet_pton(AF_INET6, oai::utils::trim(ipv6_addr_str).c_str(),
                  buf_in6_addr) == 1) {
      str->slen = 16;
...
void oai::utils::ipv4v6_to_pdu_address_information(struct in_addr ipv4_address,
...
bool oai::utils::pdu_address_information_to_ipv4v6(
  ...
  if ((str->slen != size_str) or (str->data == nullptr))
    return false;
  // 4 octets for IPv4
...
void oai::utils::ipv4v6_to_transport_layer_address(struct in_addr ipv4_address,
                                                   struct in6_addr ipv6_address,
                                                   bstring str) {
