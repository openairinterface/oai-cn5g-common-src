/**
* NRF NFManagement Service
* NRF NFManagement Service.   © 2024, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.7
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#include "TsctsfInfo.h"
#include "Helpers.h"

#include <sstream>

namespace oai::model
{

TsctsfInfo::TsctsfInfo()
{
    m_SNssaiInfoListIsSet = false;
    m_ExternalGroupIdentifiersRangesIsSet = false;
    m_SupiRangesIsSet = false;
    m_GpsiRangesIsSet = false;
    m_InternalGroupIdentifiersRangesIsSet = false;
    
}

void TsctsfInfo::validate() const
{
    std::stringstream msg;
    if (!validate(msg))
    {
        throw org::openapitools::server::helpers::ValidationException(msg.str());
    }
}

bool TsctsfInfo::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool TsctsfInfo::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "TsctsfInfo" : pathPrefix;

         
    if (sNssaiInfoListIsSet())
    {
        const std::map<std::string, oai::model::SnssaiTsctsfInfoItem>& value = m_SNssaiInfoList;
        const std::string currentValuePath = _pathPrefix + ".sNssaiInfoList";
                
        

    }
         
    if (externalGroupIdentifiersRangesIsSet())
    {
        const std::vector<oai::model::IdentityRange>& value = m_ExternalGroupIdentifiersRanges;
        const std::string currentValuePath = _pathPrefix + ".externalGroupIdentifiersRanges";
                
        
        if (value.size() < 1)
        {
            success = false;
            msg << currentValuePath << ": must have at least 1 elements;";
        }
        { // Recursive validation of array elements
            const std::string oldValuePath = currentValuePath;
            int i = 0;
            for (const oai::model::IdentityRange& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".externalGroupIdentifiersRanges") && success;
 
                i++;
            }
        }

    }
         
    if (supiRangesIsSet())
    {
        const std::vector<oai::model::SupiRange>& value = m_SupiRanges;
        const std::string currentValuePath = _pathPrefix + ".supiRanges";
                
        
        if (value.size() < 1)
        {
            success = false;
            msg << currentValuePath << ": must have at least 1 elements;";
        }
        { // Recursive validation of array elements
            const std::string oldValuePath = currentValuePath;
            int i = 0;
            for (const oai::model::SupiRange& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".supiRanges") && success;
 
                i++;
            }
        }

    }
         
    if (gpsiRangesIsSet())
    {
        const std::vector<oai::model::IdentityRange>& value = m_GpsiRanges;
        const std::string currentValuePath = _pathPrefix + ".gpsiRanges";
                
        
        if (value.size() < 1)
        {
            success = false;
            msg << currentValuePath << ": must have at least 1 elements;";
        }
        { // Recursive validation of array elements
            const std::string oldValuePath = currentValuePath;
            int i = 0;
            for (const oai::model::IdentityRange& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".gpsiRanges") && success;
 
                i++;
            }
        }

    }
         
    if (internalGroupIdentifiersRangesIsSet())
    {
        const std::vector<oai::model::InternalGroupIdRange>& value = m_InternalGroupIdentifiersRanges;
        const std::string currentValuePath = _pathPrefix + ".internalGroupIdentifiersRanges";
                
        
        if (value.size() < 1)
        {
            success = false;
            msg << currentValuePath << ": must have at least 1 elements;";
        }
        { // Recursive validation of array elements
            const std::string oldValuePath = currentValuePath;
            int i = 0;
            for (const oai::model::InternalGroupIdRange& value : value)
            { 
                const std::string currentValuePath = oldValuePath + "[" + std::to_string(i) + "]";
                        
        success = value.validate(msg, currentValuePath + ".internalGroupIdentifiersRanges") && success;
 
                i++;
            }
        }

    }
    
    return success;
}

bool TsctsfInfo::operator==(const TsctsfInfo& rhs) const
{
    return
    
    
    
    ((!sNssaiInfoListIsSet() && !rhs.sNssaiInfoListIsSet()) || (sNssaiInfoListIsSet() && rhs.sNssaiInfoListIsSet() && getSNssaiInfoList() == rhs.getSNssaiInfoList())) &&
    
    
    ((!externalGroupIdentifiersRangesIsSet() && !rhs.externalGroupIdentifiersRangesIsSet()) || (externalGroupIdentifiersRangesIsSet() && rhs.externalGroupIdentifiersRangesIsSet() && getExternalGroupIdentifiersRanges() == rhs.getExternalGroupIdentifiersRanges())) &&
    
    
    ((!supiRangesIsSet() && !rhs.supiRangesIsSet()) || (supiRangesIsSet() && rhs.supiRangesIsSet() && getSupiRanges() == rhs.getSupiRanges())) &&
    
    
    ((!gpsiRangesIsSet() && !rhs.gpsiRangesIsSet()) || (gpsiRangesIsSet() && rhs.gpsiRangesIsSet() && getGpsiRanges() == rhs.getGpsiRanges())) &&
    
    
    ((!internalGroupIdentifiersRangesIsSet() && !rhs.internalGroupIdentifiersRangesIsSet()) || (internalGroupIdentifiersRangesIsSet() && rhs.internalGroupIdentifiersRangesIsSet() && getInternalGroupIdentifiersRanges() == rhs.getInternalGroupIdentifiersRanges()))
    
    ;
}

bool TsctsfInfo::operator!=(const TsctsfInfo& rhs) const
{
    return !(*this == rhs);
}

void to_json(nlohmann::json& j, const TsctsfInfo& o)
{
    j = nlohmann::json::object();
    if(o.sNssaiInfoListIsSet() || !o.m_SNssaiInfoList.empty())
        j["sNssaiInfoList"] = o.m_SNssaiInfoList;
    if(o.externalGroupIdentifiersRangesIsSet() || !o.m_ExternalGroupIdentifiersRanges.empty())
        j["externalGroupIdentifiersRanges"] = o.m_ExternalGroupIdentifiersRanges;
    if(o.supiRangesIsSet() || !o.m_SupiRanges.empty())
        j["supiRanges"] = o.m_SupiRanges;
    if(o.gpsiRangesIsSet() || !o.m_GpsiRanges.empty())
        j["gpsiRanges"] = o.m_GpsiRanges;
    if(o.internalGroupIdentifiersRangesIsSet() || !o.m_InternalGroupIdentifiersRanges.empty())
        j["internalGroupIdentifiersRanges"] = o.m_InternalGroupIdentifiersRanges;
    
}

void from_json(const nlohmann::json& j, TsctsfInfo& o)
{
    if(j.find("sNssaiInfoList") != j.end())
    {
        j.at("sNssaiInfoList").get_to(o.m_SNssaiInfoList);
        o.m_SNssaiInfoListIsSet = true;
    } 
    if(j.find("externalGroupIdentifiersRanges") != j.end())
    {
        j.at("externalGroupIdentifiersRanges").get_to(o.m_ExternalGroupIdentifiersRanges);
        o.m_ExternalGroupIdentifiersRangesIsSet = true;
    } 
    if(j.find("supiRanges") != j.end())
    {
        j.at("supiRanges").get_to(o.m_SupiRanges);
        o.m_SupiRangesIsSet = true;
    } 
    if(j.find("gpsiRanges") != j.end())
    {
        j.at("gpsiRanges").get_to(o.m_GpsiRanges);
        o.m_GpsiRangesIsSet = true;
    } 
    if(j.find("internalGroupIdentifiersRanges") != j.end())
    {
        j.at("internalGroupIdentifiersRanges").get_to(o.m_InternalGroupIdentifiersRanges);
        o.m_InternalGroupIdentifiersRangesIsSet = true;
    } 
    
}

std::map<std::string, oai::model::SnssaiTsctsfInfoItem> TsctsfInfo::getSNssaiInfoList() const
{
    return m_SNssaiInfoList;
}
void TsctsfInfo::setSNssaiInfoList(std::map<std::string, oai::model::SnssaiTsctsfInfoItem> const& value)
{
    m_SNssaiInfoList = value;
    m_SNssaiInfoListIsSet = true;
}
bool TsctsfInfo::sNssaiInfoListIsSet() const
{
    return m_SNssaiInfoListIsSet;
}
void TsctsfInfo::unsetSNssaiInfoList()
{
    m_SNssaiInfoListIsSet = false;
}
std::vector<oai::model::IdentityRange> TsctsfInfo::getExternalGroupIdentifiersRanges() const
{
    return m_ExternalGroupIdentifiersRanges;
}
void TsctsfInfo::setExternalGroupIdentifiersRanges(std::vector<oai::model::IdentityRange> const& value)
{
    m_ExternalGroupIdentifiersRanges = value;
    m_ExternalGroupIdentifiersRangesIsSet = true;
}
bool TsctsfInfo::externalGroupIdentifiersRangesIsSet() const
{
    return m_ExternalGroupIdentifiersRangesIsSet;
}
void TsctsfInfo::unsetExternalGroupIdentifiersRanges()
{
    m_ExternalGroupIdentifiersRangesIsSet = false;
}
std::vector<oai::model::SupiRange> TsctsfInfo::getSupiRanges() const
{
    return m_SupiRanges;
}
void TsctsfInfo::setSupiRanges(std::vector<oai::model::SupiRange> const& value)
{
    m_SupiRanges = value;
    m_SupiRangesIsSet = true;
}
bool TsctsfInfo::supiRangesIsSet() const
{
    return m_SupiRangesIsSet;
}
void TsctsfInfo::unsetSupiRanges()
{
    m_SupiRangesIsSet = false;
}
std::vector<oai::model::IdentityRange> TsctsfInfo::getGpsiRanges() const
{
    return m_GpsiRanges;
}
void TsctsfInfo::setGpsiRanges(std::vector<oai::model::IdentityRange> const& value)
{
    m_GpsiRanges = value;
    m_GpsiRangesIsSet = true;
}
bool TsctsfInfo::gpsiRangesIsSet() const
{
    return m_GpsiRangesIsSet;
}
void TsctsfInfo::unsetGpsiRanges()
{
    m_GpsiRangesIsSet = false;
}
std::vector<oai::model::InternalGroupIdRange> TsctsfInfo::getInternalGroupIdentifiersRanges() const
{
    return m_InternalGroupIdentifiersRanges;
}
void TsctsfInfo::setInternalGroupIdentifiersRanges(std::vector<oai::model::InternalGroupIdRange> const& value)
{
    m_InternalGroupIdentifiersRanges = value;
    m_InternalGroupIdentifiersRangesIsSet = true;
}
bool TsctsfInfo::internalGroupIdentifiersRangesIsSet() const
{
    return m_InternalGroupIdentifiersRangesIsSet;
}
void TsctsfInfo::unsetInternalGroupIdentifiersRanges()
{
    m_InternalGroupIdentifiersRangesIsSet = false;
}


} // namespace oai::model

