...
asn_enc_rval_t OPEN_TYPE_encode_oer(
  ...
  if (td->elements_count == 0) {
    ...
    ASN_DEBUG("ERROR: OPEN_TYPE_encode_oer called in direct type mode");
    ASN__ENCODE_FAILED;
  }
...
asn_enc_rval_t OPEN_TYPE_oer_put(
    ...
    const asn_TYPE_member_t* elm, asn_app_consume_bytes_f* cb, void* app_key) {
  asn_type_selector_result_t selected;
  const void* memb_ptr;
  ssize_t encoded;
  asn_enc_rval_t er = {0, 0, 0};
  ...
  if (!elm->type->op || !elm->type->op->oer_encoder) {
    ASN_DEBUG(
        "Open Type %s->%s: OER encoder is not defined", td->name, elm->name);
    ASN__ENCODE_FAILED;
  }
  ...
  if (!selected.type_descriptor->op ||
      (!elm->type->elements_count &&
       !selected.type_descriptor->op->oer_encoder)) {
    ASN_DEBUG(
        "Open Type %s->%s: selected type %s has no OER encoder", td->name,
        elm->name, selected.type_descriptor->name);
    ASN__ENCODE_FAILED;
  }

  ASN_DEBUG(
      "OPEN_TYPE_oer_put: elm->type=%s, elements=%p, elements_count=%u, "
  ...
  if (elm->type->elements_count > 0) {
     ...
     * OER encoder. The encoder will handle the choice index and variant
    ...
    if ((encoded = oer_open_type_put(
             selected.type_descriptor,
             elm->encoding_constraints.oer_constraints, memb_ptr, cb,
             app_key)) < 0) {
      ASN__ENCODE_FAILED;
    }
    er.encoded = encoded;
