...
bool mime_parser::parse(const std::string &str) {
  ...
  oai::logger::logger_common::common().debug("Boundary: %s",
                                             boundary_str.c_str());
  ...
  while (boundary_pos < boundary_last_post) {
    ...
    if ((content_type_pos == std::string::npos) or
      ...
      break;
    p.content_type = str_tmp.substr(content_type_pos + 14,
                                    crlf_pos - (content_type_pos + 14));
    oai::logger::logger_common::common().debug("Content Type: %s",
                                               p.content_type.c_str());
    ...
    oai::logger::logger_common::common().debug("Content Id: %s",
                                               p.content_id.c_str());
...
//------------------------------------------------------------------------------
uint8_t mime_parser::parse(std::string input, std::string &jsonData,
...
//------------------------------------------------------------------------------
void mime_parser::get(const std::string &content_id,
...
unsigned char *mime_parser::format_string_as_hex(const std::string &str) {
  ...
  oai::logger::logger_common::common().debug("Input string (%d bytes): %s ",
                                             str_len, str.c_str());
