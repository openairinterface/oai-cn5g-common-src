...
void PduSessionResourceModifyResponseMsg::initialize() {
  ...
  if (!pduSessionResourceModifyResponseIes->protocolIEs) {
    pduSessionResourceModifyResponseIes->protocolIEs =
...
bool PduSessionResourceModifyResponseMsg::decode(Ngap_NGAP_PDU_t* ngapMsgPdu) {
  ...
  for (int i = 0;
    ...
    Ngap_PDUSessionResourceModifyResponseIEs_t* ngap_ie =
    ...
    switch (ngap_ie->id) {
      ...
      case Ngap_ProtocolIE_ID_id_PDUSessionResourceModifyListModRes: {
        if (ngap_ie->criticality == Ngap_Criticality_ignore &&
          ...
          PduSessionResourceModifyListModRes item_list = {};
          if (!item_list.decode(
                  ngap_ie->value.choice.PDUSessionResourceModifyListModRes)) {
            oai::logger::logger_common::ngap().error(
