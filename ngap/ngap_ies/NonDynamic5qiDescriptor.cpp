...
bool NonDynamic5qiDescriptor::encode(
  ...
  if (!m_FiveQI.encode(nonDynamic5qiDescriptor.fiveQI))
    return false;
  if (m_PriorityLevelQos.has_value()) {
    ...
    if (!plq)
      return false;
    if (!m_PriorityLevelQos.value().encode(*plq))
      return false;
