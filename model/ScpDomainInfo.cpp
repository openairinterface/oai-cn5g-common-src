...
bool ScpDomainInfo::validate(
    std::stringstream& msg, const std::string& pathPrefix) const {
  bool success = true;
  const std::string _pathPrefix =
      pathPrefix.empty() ? "ScpDomainInfo" : pathPrefix;

  if (scpFqdnIsSet()) {
    const std::string& value           = m_ScpFqdn;
    const std::string currentValuePath = _pathPrefix + ".scpFqdn";
  ...
  if (scpIpEndPointsIsSet()) {
    const std::vector<oai::_3gpp::model::IpEndPoint>& value = m_ScpIpEndPoints;
    ...
    {  // Recursive validation of array elements
      ...
      for (const oai::_3gpp::model::IpEndPoint& value : value) {
        ...
        success = value.validate(msg, currentValuePath + ".scpIpEndPoints") &&
                  success;
...
bool ScpDomainInfo::operator==(const ScpDomainInfo& rhs) const {
  return
      ...
      ((!scpFqdnIsSet() && !rhs.scpFqdnIsSet()) ||
       (scpFqdnIsSet() && rhs.scpFqdnIsSet() &&
        getScpFqdn() == rhs.getScpFqdn())) &&

      ((!scpIpEndPointsIsSet() && !rhs.scpIpEndPointsIsSet()) ||
       (scpIpEndPointsIsSet() && rhs.scpIpEndPointsIsSet() &&
        getScpIpEndPoints() == rhs.getScpIpEndPoints())) &&
...
void to_json(nlohmann::json& j, const ScpDomainInfo& o) {
  j = nlohmann::json();
  if (o.scpFqdnIsSet()) j["scpFqdn"] = o.m_ScpFqdn;
...
std::vector<oai::_3gpp::model::IpEndPoint> ScpDomainInfo::getScpIpEndPoints()
    const {
  return m_ScpIpEndPoints;
}
void ScpDomainInfo::setScpIpEndPoints(
