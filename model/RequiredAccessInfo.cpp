/**
* Npcf_PolicyAuthorization Service API
* PCF Policy Authorization Service.   © 2023, 3GPP Organizational Partners (ARIB, ATIS, CCSA, ETSI, TSDSI, TTA, TTC).   All rights reserved. 
*
* The version of the OpenAPI document: 1.2.3
* 
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/


#include "RequiredAccessInfo.h"
#include "Helpers.h"

#include <sstream>

namespace oai::_3gpp::model
{

RequiredAccessInfo::RequiredAccessInfo()
{
    
}

void RequiredAccessInfo::validate() const
{
    std::stringstream msg;
    if (!validate(msg))
    {
        throw oai::_3gpp::model::helpers::ValidationException(msg.str());
    }
}

bool RequiredAccessInfo::validate(std::stringstream& msg) const
{
    return validate(msg, "");
}

bool RequiredAccessInfo::validate(std::stringstream& msg, const std::string& pathPrefix) const
{
    bool success = true;
    const std::string _pathPrefix = pathPrefix.empty() ? "RequiredAccessInfo" : pathPrefix;

    
    if (!m_value.validate(msg))
    {
        success = false;
    }
    return success;
}

bool RequiredAccessInfo::operator==(const RequiredAccessInfo& rhs) const
{
    return
    
    getValue() == rhs.getValue()
    ;
}

bool RequiredAccessInfo::operator!=(const RequiredAccessInfo& rhs) const
{
    return !(*this == rhs);
}

void to_json(nlohmann::json& j, const RequiredAccessInfo& o)
{
    j = nlohmann::json();
    to_json(j, o.m_value);
}

void from_json(const nlohmann::json& j, RequiredAccessInfo& o)
{
    from_json(j, o.m_value);
}

RequiredAccessInfo_anyOf RequiredAccessInfo::getValue() const
{
    return m_value;
}

void RequiredAccessInfo::setValue(RequiredAccessInfo_anyOf value)
{
    m_value = value;
}

RequiredAccessInfo_anyOf::eRequiredAccessInfo_anyOf RequiredAccessInfo::getEnumValue() const
{
    return m_value.getValue();
}

void RequiredAccessInfo::setEnumValue(RequiredAccessInfo_anyOf::eRequiredAccessInfo_anyOf  value)
{
    m_value.setValue(value);
}

} // namespace oai::_3gpp::model

