 ...
 */
static ssize_t oer_fetch_quantity(const void *ptr, size_t size, size_t *qty_r) {
  const uint8_t *b;
  const uint8_t *bend;
  size_t len = 0;
  size_t qty;
...
asn_dec_rval_t SET_OF_decode_oer(const asn_codec_ctx_t *opt_codec_ctx,
...
static ssize_t oer_put_quantity(size_t qty, asn_app_consume_bytes_f *cb,
                                void *app_key) {
 ...
 */
asn_enc_rval_t SET_OF_encode_oer(const asn_TYPE_descriptor_t *td,
                                 const asn_oer_constraints_t *constraints,
