 ...
 */
asn_dec_rval_t SEQUENCE_decode_xer(
    const asn_codec_ctx_t* opt_codec_ctx, const asn_TYPE_descriptor_t* td,
    void** struct_ptr, const char* opt_mname, const void* ptr, size_t size) {
  /*
  ...
  for (edx = ctx->step; ctx->phase <= 3;) {
    ...
    if (ctx->phase == 2) {
      ...
      if (elm->flags & ATF_OPEN_TYPE) {
        ...
        /* Debug: Check if CHOICE present field was set (only in CHOICE wrapper
         * mode) */
        if (tmprval.code == RC_OK && elm->type->elements_count > 0) {
          void* choice_ptr = (elm->flags & ATF_POINTER) ?
                                 *(void**) ((char*) st + elm->memb_offset) :
                                 (void*) ((char*) st + elm->memb_offset);
          if (choice_ptr) {
            unsigned int* present __attribute__((unused)) =
                (unsigned int*) choice_ptr;
        ...
        tmprval = elm->type->op->xer_decoder(
            opt_codec_ctx, elm->type, memb_ptr2, elm->name, ptr, size);
      }
     ...
     */
    ch_size = xer_next_token(&ctx->context, ptr, size, &ch_type);
    if (ch_size == -1) {
      RETURN(RC_FAIL);
    } else {
      switch (ch_type) {
    ...
    switch (tcv) {
      ...
      case XCT_UNKNOWN_BO:
        ...
        ASN_DEBUG(
            "XER/SEQUENCE: tcv=%d, ph=%d, edx=%" ASN_PRI_SIZE "", tcv,
            ctx->phase, edx);
        ...
        if (ctx->phase == 0) {
          ...
          if (seq_tcv == XCT_OPENING || seq_tcv == XCT_BOTH) {
            ASN_DEBUG(
                "XER/SEQUENCE: Accepting generic <SEQUENCE> tag in phase 0");
            XER_ADVANCE(ch_size);
            ctx->phase = 1; /* Processing body phase */
            continue;
          }
        ...
        if (edx < td->elements_count) {
          ...
          for (n = edx; n < edx_end; n++) {
            ...
            ASN_DEBUG(
                "XER/SEQUENCE: Checking member n=%zu, name='%s', tcv=%d, "
                "tag='%.*s'",
            ...
            switch (tcv) {
              case XCT_BOTH:
                ...
                if (elm->optional) {
                  ASN_DEBUG(
                      "XER/SEQUENCE: Empty optional field '%s', treating as "
                      "absent",
              ...
              case XCT_OPENING:
                ...
                if (elm->optional) {
                  ...
                  while (peek_size > 0) {
                    peek_ch_size = xer_next_token(
                        &peek_ctx, peek_ptr, peek_size, &peek_type);
                    if (peek_ch_size <= 0) break;

                    if (peek_type == PXER_COMMENT || peek_type == PXER_TEXT) {
                      /* Skip whitespace/comments */
                      size_t ws_span =
                          xer_whitespace_span(peek_ptr, peek_ch_size);
                    ...
                    if (peek_type == PXER_TAG) {
                      xer_check_tag_e peek_tcv =
                          xer_check_tag(peek_ptr, peek_ch_size, elm->name);
                      if (peek_tcv == XCT_CLOSING) {
                        /* This is an empty optional field! */
                        ASN_DEBUG(
                            "XER/SEQUENCE: Empty optional field '%s' (separate "
                            "tags), treating as absent",
                            elm->name ? elm->name : "(null)");
                        /* Capture closing tag size before using it */
                        closing_tag_size = peek_ch_size;
                        /* Skip both opening and closing tags */
                        XER_ADVANCE(
                            (peek_ptr - (const char*) ptr) + closing_tag_size);
                        ctx->step = edx = n + 1;
                        break; /* Exit inner loop to get next token */
                      }
                    }
                  ...
                  /* If we found and handled an empty optional, skip normal
                   * processing */
          ...
          ASN_DEBUG(
              "Out of defined members: %" ASN_PRI_SIZE "/%u", edx,
              td->elements_count);
        }
        ...
        if (IN_EXTENSION_GROUP(
                specs,
                edx +
                    (edx < td->elements_count ? elements[edx].optional : 0))) {
          ASN_DEBUG("Got anticipated extension at %" ASN_PRI_SIZE "", edx);
          /*
        ...
        /* Check if this is a type wrapper tag (e.g., <Reset> when
         * opt_mname="value") */
        if (ctx->phase == 1 && td->xml_tag && opt_mname &&
          ...
          if (tcv == XCT_OPENING || tcv == XCT_BOTH) {
            ASN_DEBUG(
                "XER/SEQUENCE: Skipping type wrapper tag <%s>", td->xml_tag);
            XER_ADVANCE(ch_size);
        ...
        /* Fall through */
      case XCT_UNKNOWN_CL:
        /* Check if this is a type wrapper closing tag (e.g., </Reset> when
         * opt_mname="value") */
        if (ctx->phase == 1 && td->xml_tag && opt_mname &&
            strcmp(td->xml_tag, opt_mname) != 0) {
          xer_check_tag_e wrapper_tcv =
              xer_check_tag(ptr, ch_size, td->xml_tag);
          if (wrapper_tcv == XCT_CLOSING) {
            ASN_DEBUG(
                "XER/SEQUENCE: Closing type wrapper tag </%s>", td->xml_tag);
            XER_ADVANCE(ch_size);
        ...
        if (ctx->phase == 1) {
          ...
          if (seq_tcv == XCT_CLOSING) {
            ASN_DEBUG(
                "XER/SEQUENCE: Accepting generic </SEQUENCE> tag in phase 1");
            ...
            if (edx >= td->elements_count ||
              ...
              if (opt_mname && td->xml_tag &&
                  strcmp(opt_mname, td->xml_tag) != 0) {
                /* Element name differs from type name, so expect outer
                 * </opt_mname> tag.
                 * Continue looping to consume it in the next iteration. */
                ASN_DEBUG(
                    "XER/SEQUENCE: Expecting outer closing tag </%s>",
                    opt_mname);
                continue;
              } else {
...
asn_enc_rval_t SEQUENCE_encode_xer(
  ...
  for (edx = 0; edx < td->elements_count; edx++) {
    ...
    if (!xcan) {
       ...
       * Primitive types like INTEGER output inline content, so no indent
       * needed. */
      if (tmper.encoded > 0 &&
