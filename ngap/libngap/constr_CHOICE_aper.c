...
asn_dec_rval_t CHOICE_decode_aper(const asn_codec_ctx_t *opt_codec_ctx,
  ...
  if (ct && ct->range_bits >= 0) {
    ...
    if (specs && specs->tag2el_count > specs->ext_start) {
      value = aper_get_nsnnwn(
          pd, specs->tag2el_count -
                  specs->ext_start); /* extension elements range */
      if (value < 0)
        ASN__DECODE_STARVED;
...
asn_enc_rval_t CHOICE_encode_aper(const asn_TYPE_descriptor_t *td,
                                  const asn_per_constraints_t *constraints,
