...
ssize_t aper_get_length(
    asn_per_data_t* pd, ssize_t lb, ssize_t ub, int ebits, int* repeat) {
...
ssize_t aper_get_nsnnwn(asn_per_data_t* pd) {
  ...
  /* X.691 2002 10.6.1 */
  if (b == 0) return per_get_few_bits(pd, 6);
...
long aper_get_constrained_whole_number(asn_per_data_t* pd, long lb, long ub) {
  assert(ub >= lb);
  long range = ub - lb + 1;
  int range_len;
  int value_len;
  ...
  if (range <= 255) {
    ...
    if (value < 0 || value >= range) return -1;
    return value + lb;
  }
...
ssize_t aper_put_length(
    asn_per_outp_t* po, ssize_t lb, ssize_t ub, size_t n, int* need_eom) {
  int constrained = (lb >= 0) && (ub >= 0);
  int dummy       = 0;
  ...
  *need_eom = 0;
  ...
  /* X.691 2002 10.9.3.3 */
  if (constrained && ub < 65536)
    return aper_put_constrained_whole_number(po, lb, ub, n + lb) ? -1 :
                                                                   (ssize_t) n;

  if (aper_put_align(po) < 0) return -1;

  if (n <= 127) { /* #11.9.3.6 */
    return per_put_few_bits(po, n, 8) ? -1 : (ssize_t) n;
  } else if (n < 16384) /* #11.9.3.7 */
    return per_put_few_bits(po, n | 0x8000, 16) ? -1 : (ssize_t) n;
  ...
  *need_eom = 0 == (n & 16383);
  ...
  return per_put_few_bits(po, 0xC0 | n, 8) ? -1 : (ssize_t) (n << 14);
}
...
int aper_put_nsnnwn(asn_per_outp_t* po, int number) {
  ...
  if (number <= 63) {
    if (per_put_few_bits(po, 0, 1)) return -1;
  ...
  if (len <= 127) {
    if (per_put_few_bits(po, 0, 1)) return -1;
    if (per_put_few_bits(po, len, 7)) return -1;
  } else {
...
int aper_put_constrained_whole_number(
    asn_per_outp_t* po, long lb, long ub, long number) {
  assert(ub >= lb);
  ...
  ASN_DEBUG(
      "aper put constrained_whole_number %ld with lb %ld and ub %ld", number,
      lb, ub);

  if (number < lb || number > ub) return -1;
  ...
  if (aper_put_constrained_whole_number(po, 1, range_len, value_len)) return -1;
  if (aper_put_align(po)) return -1;
