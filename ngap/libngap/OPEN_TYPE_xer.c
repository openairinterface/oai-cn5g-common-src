...
asn_dec_rval_t OPEN_TYPE_xer_get(
  ...
  ASN_DEBUG(
      "OPEN_TYPE_xer_get: elm->type=%s, elements=%p, elements_count=%u, "
  ...
  switch (xer_check_tag(ptr, ch_size, elm->name)) {
    case XCT_BOTH:
      ...
      if (elm->optional) {
        ASN_DEBUG(
            "OPEN_TYPE: Empty optional field '%s', treating as absent",
  ...
  rv.consumed = 0;
  ASN_DEBUG(
      "xer_decoder returned code=%d for %s", rv.code,
...
asn_enc_rval_t OPEN_TYPE_xer_put(
    ...
    const asn_TYPE_member_t* elm, int ilevel, enum xer_encoder_flags_e flags,
  ...
  ASN_DEBUG(
      "OPEN_TYPE_xer_put: elm->type=%s, elements=%p, elements_count=%u, "
      "selected.presence_index=%u, selected.type=%s",
      elm->type->name, (void*) elm->type->elements, elm->type->elements_count,
      selected.presence_index, selected.type_descriptor->name);
  ...
  if (elm->type->elements_count > 0) {
     ...
     * tag */
    ASN_DEBUG(
        "Direct type mode: encoding using %s", selected.type_descriptor->name);
    ...
    /* Check if type_name contains ASN.1 meta-syntax keywords that should not be
     * output as wrapper tags */
    ...
    tmper = selected.type_descriptor->op->xer_encoder(
        selected.type_descriptor, memb_ptr, skip_wrapper ? ilevel : ilevel + 1,
        flags, cb, app_key);
