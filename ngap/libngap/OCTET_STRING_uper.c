...
asn_dec_rval_t OCTET_STRING_decode_uper(
  ...
  if (csiz->effective_bits == 0) {
    ...
    if (bpc) {
      ...
      ret = OCTET_STRING_per_get_characters(pd, st->buf, csiz->upper_bound, bpc,
  ...
  do {
    ...
    /* Get the PER length */
    raw_len =
        uper_get_length(pd, csiz->effective_bits, csiz->lower_bound, &repeat);
    if (raw_len < 0)
      RETURN(RC_WMORE);
    if (raw_len == 0 && st->buf)
      break;
    ...
    ret = OCTET_STRING_per_get_characters(pd, &st->buf[st->size], raw_len, bpc,
                                          unit_bits, cval->lower_bound,
                                          cval->upper_bound, pc);
    if (ret > 0)
      RETURN(RC_FAIL);
...
OCTET_STRING_encode_uper(const asn_TYPE_descriptor_t *td,
  ...
  ASN_DEBUG("Encoding %s into %" ASN_PRI_SIZE " units of %d bits"
            " (%ld..%ld, effective %d)%s",
            td->name, size_in_units, unit_bits, csiz->lower_bound,
            csiz->upper_bound, csiz->effective_bits,
            ct_extensible ? " EXT" : "");
  ...
  if (csiz->effective_bits >= 0 && !inext) {
    ASN_DEBUG("Encoding %" ASN_PRI_SIZE " bytes (%ld), length in %d bits",
              st->size, size_in_units - csiz->lower_bound,
              csiz->effective_bits);
  ...
  do {
    ...
    ASN_DEBUG("Encoding %" ASN_PRI_SSIZE " of %" ASN_PRI_SIZE "%s", may_save,
              size_in_units, need_eom ? ",+EOM" : "");

    ret = OCTET_STRING_per_put_characters(po, buf, may_save, bpc, unit_bits,
                                          cval->lower_bound, cval->upper_bound,
                                          pc);
    if (ret)
      ASN__ENCODE_FAILED;
