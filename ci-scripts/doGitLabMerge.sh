...
if [[ "$TARGET_COMMIT_ID" == "latest" ]]; then
...
# Restored temporary file logic exactly as original
git checkout -f "$SOURCE_COMMIT_ID" > checkout.txt 2>&1
STATUS=$(grep -Ec "fatal: reference is not a tree" checkout.txt)
rm -f checkout.txt
...
# Restored the specific egrep status check logic
STATUS=$(git status | grep -Ec "You have unmerged paths.|fix conflicts")
if [ "$STATUS" -ne 0 ]; then
    echo "There are merge conflicts.. Cannot perform further build tasks"
    STATUS=-1
fi

echo "Current Commit SHA is: $(git rev-parse HEAD)"
