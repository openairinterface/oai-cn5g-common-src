...
//------------------------------------------------------------------------------
void NrCgi::set(const std::string &mcc, const std::string &mnc,
...
bool NrCgi::encode(Ngap_NR_CGI_t &nrCgi) const {
  if (!m_PlmnId.encode(nrCgi.pLMNIdentity))
    return false;
  if (!m_NrCellIdentity.encode(nrCgi.nRCellIdentity))
    return false;
...
bool NrCgi::decode(const Ngap_NR_CGI_t &nrCgi) {
  if (!m_PlmnId.decode(nrCgi.pLMNIdentity))
    return false;
  if (!m_NrCellIdentity.decode(nrCgi.nRCellIdentity))
    return false;
  return true;
}
